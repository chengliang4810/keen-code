...
RUN --mount=type=cache,id=keencode-bench-cargo-registry,target=/usr/local/cargo/registry \
    ...
    --mount=type=cache,id=keencode-bench-target,target=/source/target \
    cargo build --manifest-path Cargo.toml -p keencode-desktop \
        --features benchmark --example keencode-bench --release \
    && mkdir -p /out \
    && cp /source/target/release/examples/keencode-bench /out/keencode-bench
