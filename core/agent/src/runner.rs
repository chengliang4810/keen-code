...
const GOAL_CONTINUATION_INSTRUCTION: &str = "At this runtime boundary, this task owns the active goal below. Continue useful work within the user's authorized scope while this goal remains active. Avoid repeating completed work; choose the next concrete action toward the objective. Before completing the goal, audit the actual current state: restate the objective as concrete deliverables or success criteria; map every explicit requirement, numbered item, named file, command, test, gate, and deliverable to concrete evidence; inspect the relevant artifacts and results for each item. Verify that a test suite, manifest, verifier, or green status actually covers the requirements before relying on it. Passing tests, effort spent, partial progress, a plausible final answer, or a completed plan are not proof of completion by themselves; a plan is sufficient only when the requested goal was to produce that plan. Identify missing, incomplete, weakly verified, or uncovered requirements. Treat unresolved uncertainty as not achieved: verify further or continue authorized work. A final response alone does not complete the goal: use Goal complete with evidence for every requirement when it is actually achieved, or Goal block with a specific reason when progress requires user input or an external change after recoverable issues have been investigated. Follow new user instructions and the latest goal state; do not adopt a replacement goal, invent further work, or expand authorization.";
