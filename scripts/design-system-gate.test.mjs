...
test("design-system gate rejects visible native controls", () => {
  const violations = inspectSource("src/components/Example.tsx", "export function Example() { return <button>保存</button>; }");
...
test("design-system gate reports raw colors and CSS literals", () => {
  const tsViolations = inspectSource("src/components/Example.tsx", 'const color = "#fff";');
  const cssViolations = inspectSource("src/components/example.css", ".x { color: #fff; }");
...
test("design-system gate honors explicit token and host allowlists", () => {
  assert.deepEqual(inspectSource("src/styles/tokens.css", ":root { --brand: #fff; }"), []);
  assert.deepEqual(inspectSource("src/components/TerminalPanel.tsx", 'const theme = { background: "#000" };'), []);
});
