...
日志在本机保存，常见凭据字段脱敏，换行转义，单条文本上限约 4 KB。当前文件达到 8 MiB 后轮转为 `.log.1` 和带时间戳的副本，备份按最近 7 天且总量不超过 256 MiB 清理。普通日志进入容量为 1,024 的有界队列，由专用写入线程顺序落盘；队列满载时丢弃并累加 `diagnostics.dropped_logs`，调用方不因普通日志写入阻塞。Crash 摘要同步追加到 `.crash.jsonl` spool，文件同时受 128 KiB 和 32 条记录上限约束，启动时只恢复最近记录。前端资源采样只在可见的活跃或观测面板模式运行，分别按 5 秒或 1 秒取样，隐藏窗口和卸载时清理；没有日志上传。使用标准 `tracing-subscriber` 的最小 fmt/registry/std 功能；本次未做新的安装包、CPU、内存或启动性能基准，不据此声称性能改善。
