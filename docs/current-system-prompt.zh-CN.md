...
项目指令按 `AGENTS.md`、`CLAUDE.md`、`.claude/AGENTS.md` 的顺序选择第一个存在的主文件，再追加 `CLAUDE.local.md`。两份非空正文仅以空行连接，不添加说明、标题或路径标签；没有主文件时也可单独加载本地文件。每轮重新读取，放在静态系统提示词中的全局指令之后。此处采用 Peri 的文件选择与原文拼接方式，不解析 `@import`。
