...
/// host 装配输入：调用方（cli/TUI/print）持有的轻量输入。
///
...
/// 本装配面内部构造
/// ——「ACP Host = 部署单元」，TUI/print 只提供协议面输入
/// （provider / config / thread_store / cwd），不再直接触碰
...
pub struct HostAssemblyInput {
    ...
    pub bare: bool,
    /// 驱动 cron tick（TUI=true，复刻迁移前 TUI 每秒 tick 行为；print
    /// 保持现状无 tick）。
...
/// Settings Hooks 则注入现有端口实现；这里只构造实现对象，不读取配置。
/// global/project/local Hooks 会在每轮执行前按该会话的 cwd 加载，不在启动阶段
/// 固定为桌面进程目录。
...
/// 各 Host 路径一致的既有顺序，ARC-MIDDLEWARE-001 不重排）。
///
...
/// index、shared tools、SessionManager。
///
...
/// 边 2 assemble 路径）；行为与迁移前两路径（launch / cli_print）
