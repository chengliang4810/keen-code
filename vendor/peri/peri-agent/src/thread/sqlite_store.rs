...
use crate::{
    ...
    thread::{
        types::PendingTool, AgentStatus, CancelPolicy, CompactionLifecycle, ThreadId, ThreadMeta,
        ThreadStore,
    },
};
...
impl SqliteThreadStore {
    ...
    async fn init_schema(&self) -> Result<()> {
        ...
        // (KeenCode) replay 事件日志：event_seq 单调递增（AUTOINCREMENT），
        // 同一 thread 的消息按 event_seq 排序即得重放顺序。
        sqlx::query(
        ...
        // (KeenCode) 未完成工具调用：ToolStart 落库、ToolEnd 删除，崩溃恢复时读取。
        sqlx::query(
...
impl ThreadStore for SqliteThreadStore {
    ...
    async fn append_messages(&self, id: &ThreadId, msgs: &[BaseMessage]) -> Result<()> {
        ...
        for msg in msgs {
            ...
            .execute(&mut *tx)
            .await?;
            // (KeenCode) replay 事件日志：同一 thread 的消息按 event_seq 排序即得重放顺序。
            sqlx::query(
    ...
    // ── (KeenCode) replay 游标与 pending_tools ──────────────────────────────────
    ...
    async fn load_messages_since(
        ...
        let rows: Vec<(i64, String, String)> = if let Some(after) = after_seq {
            sqlx::query_as(
                "SELECT e.event_seq, e.message_id, m.content
    ...
    async fn latest_event_seq(&self, thread_id: &ThreadId) -> Result<i64> {
        let row: Option<(i64,)> =
            sqlx::query_as("SELECT MAX(event_seq) FROM session_events WHERE root_thread_id = ?1")
                .bind(thread_id.as_str())
                .fetch_optional(&self.pool)
                .await?;
    ...
    async fn record_pending_tool(
        ...
        sqlx::query(
            "INSERT OR REPLACE INTO pending_tools (tool_call_id, thread_id, name, input_json, started_at)
             VALUES (?1, ?2, ?3, ?4, ?5)",
        )
        .bind(tool_call_id)
        .bind(thread_id.as_str())
    ...
    async fn list_pending_tools(&self, thread_id: &ThreadId) -> Result<Vec<PendingTool>> {
        let rows: Vec<(String, String, Option<String>, String)> = sqlx::query_as(
            "SELECT tool_call_id, name, input_json, started_at
             FROM pending_tools WHERE thread_id = ?1 ORDER BY started_at ASC",
        )
        .bind(thread_id.as_str())
        .fetch_all(&self.pool)
        .await?;
