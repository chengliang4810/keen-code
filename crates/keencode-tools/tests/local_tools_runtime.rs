...
/// 失败命令必须经过真实 Agent 归一后保留退出码、预览及可读取的完整报告。
#[tokio::test]
async fn failed_command_diagnostics_survive_agent_normalization() {
    for bytes in [3499, 4999, 19999] {
        ...
        if bytes > 4096 {
            let report = text
                .lines()
                .find_map(|line| line.split_once("完整输出报告：").map(|(_, path)| path))
                .expect("大失败输出必须可定位报告");
            let full = fs::read_to_string(report).expect("模型可见报告路径必须存在");
            assert!(full.contains("HEAD") && full.contains("TAIL") && full.contains("REAL_ERROR"));
            if bytes < 16384 {
                assert!(full.len() > bytes);
            } else {
                let raw = fs::read_dir(&artifacts)
                    .unwrap()
                    .map(|e| e.unwrap().path())
                    .find(|p| {
                        p.file_name()
                    ...
                    .unwrap();
                assert_eq!(fs::read(raw).unwrap().len(), bytes + 8);
            }
        }
    }
}
