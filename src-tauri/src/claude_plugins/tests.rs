...
fn loads_default_hooks_file() {
    let root = std::env::temp_dir().join(format!("keencode-claude-hooks-{}", std::process::id()));
    ...
    .unwrap();
    let variables = BTreeMap::from([("CLAUDE_PLUGIN_ROOT".to_owned(), "/safe/plugin".to_owned())]);
...
fn saves_state_with_private_permissions() {
    let root = std::env::temp_dir().join(format!("keencode-claude-state-{}", std::process::id()));
