...
/// 当前插件公开状态文件的固定 schema 名称。
const PLUGIN_STATE_SCHEMA: &str = "keencode/plugin-state";
/// 当前插件公开状态文件的固定格式版本。
const PLUGIN_STATE_VERSION: u32 = 1;

/// 创建受控插件根目录。根目录之前的平台祖先（例如 macOS `/var`）不属于插件
/// 数据边界，只检查直接承载 `plugins` 的父级和根本身。
fn ensure_controlled_root(path: &Path, label: &str) -> Result<()> {
    ...
    let parent = path
        .parent()
        .ok_or_else(|| PluginError::Invalid(format!("{label}路径缺少父目录")))?;
    fs::create_dir_all(parent)?;
    ensure_one_directory(path, label)
}

/// 校验受控插件根及其既有祖先，不创建缺失目录。
...
fn ensure_controlled_descendant_chain(boundary: &Path, path: &Path, label: &str) -> Result<()> {
    ...
    let relative = path.strip_prefix(boundary).map_err(|_| {
        PluginError::Invalid(format!("{label}路径不在受控目录边界内：{}", path.display()))
    })?;
...
fn validate_directory_boundary(path: &Path, label: &str) -> Result<()> {
    ...
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => Err(PluginError::Invalid(format!(
            "{label}路径不允许是符号链接：{}",
            path.display()
        ))),
        Ok(metadata) if !metadata.is_dir() => Err(PluginError::Invalid(format!(
            "{label}路径不是目录：{}",
            path.display()
        ))),
        Ok(_) => Ok(()),
...
/// 判断路径是否为 macOS 固定系统目录别名，而非用户可控链接。
...
impl PluginStateFile {
    ...
    fn from_state(state: &PluginState) -> Self {
        Self {
            schema: PLUGIN_STATE_SCHEMA.to_owned(),
    ...
    /// 校验文件身份并返回当前插件状态。
    fn into_state(self) -> Result<PluginState> {
        if self.schema != PLUGIN_STATE_SCHEMA || self.version != PLUGIN_STATE_VERSION {
            return Err(PluginError::Invalid(
                "插件状态 schema 或版本不受支持".to_owned(),
            ));
        }
        Ok(self.state)
    }
...
fn secret_transaction_error(id: &PluginId, action: &str) -> PluginError {
    PluginError::Invalid(format!("插件 {id} 密钥事务{action}"))
}
...
fn cleanup_secret_generation(
    ...
    for name in names {
        let key = match storage.secret_key_at(id, name, generation) {
            Ok(key) => key,
            Err(_) => {
                tracing::warn!(plugin = %id, generation, "清理 KeenCode 插件旧代际密钥键名失败");
                continue;
            }
        };
        if secrets.delete(&key).is_err() {
            tracing::warn!(plugin = %id, generation, "清理 KeenCode 插件旧代际密钥失败");
        }
    }
}
...
impl PluginManager {
    ...
    pub fn load_state(&self) -> Result<PluginState> {
        ...
        let state = PluginStateFile::into_state(serde_json::from_slice(&read_limited(
    ...
    pub fn save_state(&self, state: &PluginState) -> Result<()> {
        ...
        let bytes = serde_json::to_vec_pretty(&PluginStateFile::from_state(state))?;
        crate::storage::atomic_write_private(&self.storage.state_path, &bytes)
            .map_err(|error| PluginError::Io(io::Error::other(error)))?;
        Ok(())
    }
    ...
    pub fn install_from_directories(
        ...
        if materialized.is_empty() {
            return Err(PluginError::Invalid("插件安装计划不能为空".to_owned()));
        }
        ...
        #[derive(Debug)]
        /// 安装提交前已经完整校验且尚未复制的单个插件计划。
        struct PreparedInstallation {
            /// 已绑定市场的稳定插件身份。
            id: PluginId,
            /// 已解析并通过 Schema 校验的插件清单。
    ...
    /// 为所有启用插件构造可交给 Agent Runtime 的只读投影。
    ...
    fn plan_user_config(
        ...
        for (name, value) in update.values {
            let definition = manifest.user_config.get(&name).ok_or_else(|| {
                PluginError::Invalid(format!("插件 {} 没有 userConfig 字段 {name}", id))
            })?;
            validate_user_config_value(&name, definition, &value)?;
...
/// 一个可加载的 KeenCode 命令、Skill 或 Agent 文件。
#[derive(Clone, Debug)]
...
pub fn interpolate_variables(input: &str, variables: &BTreeMap<String, String>) -> Result<String> {
    ...
    while let Some(start) = remainder.find("${") {
        ...
        let end = after_start
            .find('}')
            .ok_or_else(|| PluginError::Invalid("变量插值缺少结束大括号".to_owned()))?;
        ...
        {
            return Err(PluginError::Invalid(format!("变量名无效：{name}")));
        }
        let value = variables
            .get(name)
...
/// LSP 配置不得引用的会话变量；LSP 候选在项目级共享并在候选发布前启动。
const LSP_SESSION_VARIABLE_REFERENCE: &str = "${KEENCODE_SESSION_ID}";

/// 判断 JSON 的字符串键或值是否引用会话级 LSP 变量。
fn json_contains_lsp_session_variable(value: &Value) -> bool {
    match value {
        Value::String(value) => value.contains(LSP_SESSION_VARIABLE_REFERENCE),
        Value::Array(values) => values.iter().any(json_contains_lsp_session_variable),
        Value::Object(values) => values.iter().any(|(key, value)| {
            key.contains(LSP_SESSION_VARIABLE_REFERENCE)
                || json_contains_lsp_session_variable(value)
        }),
        Value::Null | Value::Bool(_) | Value::Number(_) => false,
    }
}

/// 拒绝把项目级共享的 LSP 进程错误绑定到某个 Session。
fn validate_project_scoped_lsp(server: &PluginLspServer) -> Result<()> {
    let declares_session_environment = server
        .env
        .keys()
        .any(|name| name.eq_ignore_ascii_case("KEENCODE_SESSION_ID"));
    let references_session_variable = server.command.contains(LSP_SESSION_VARIABLE_REFERENCE)
        || server
            .args
            .iter()
            .any(|argument| argument.contains(LSP_SESSION_VARIABLE_REFERENCE))
        || server.env.iter().any(|(name, value)| {
            name.contains(LSP_SESSION_VARIABLE_REFERENCE)
                || value.contains(LSP_SESSION_VARIABLE_REFERENCE)
        })
        || server
            .initialization_options
            .as_ref()
            .is_some_and(json_contains_lsp_session_variable);
    if declares_session_environment || references_session_variable {
        return Err(PluginError::Invalid(format!(
            "LSP Server {} 按项目共享，不支持会话变量 KEENCODE_SESSION_ID",
            server.name
        )));
    }
    Ok(())
...
/// 扫描插件目录并抽取 KeenCode commands/skills/agents/hooks/MCP 组件。
pub fn extract_components(
    ...
    variables.insert(
        "KEENCODE_SKILL_DIR".to_owned(),
        path_to_frontend(&root.join("skills")),
    );
    variables
        .entry("KEENCODE_SESSION_ID".to_owned())
        .or_insert_with(|| {
            environment
                .get("KEENCODE_SESSION_ID")
                .cloned()
                .unwrap_or_default()
        });
    variables.insert(
    ...
    variables.insert("KEENCODE_PLUGIN_ID".to_owned(), id.to_string());
    variables.insert("KEENCODE_PLUGIN_NAME".to_owned(), id.plugin.clone());
    if let Some(marketplace) = &id.marketplace {
        variables.insert(
            "KEENCODE_PLUGIN_MARKETPLACE".to_owned(),
            marketplace.clone(),
        );
    }
    if let Some(version) = &manifest.version {
        variables.insert("KEENCODE_PLUGIN_VERSION".to_owned(), version.clone());
    }
    for (name, value) in &config.values {
    ...
    let lsp_servers = manifest
        ...
        .map(|server| {
            ...
            environment.insert("KEENCODE_PLUGIN_ROOT".to_owned(), path_to_frontend(&root));
            Ok(RuntimeLspServer {
                name: format!("{plugin_namespace}:{}", server.name),
                command,
                args,
                ...
                max_restarts: server.max_restarts.unwrap_or(3),
                startup_timeout_ms: server.startup_timeout.unwrap_or(30_000),
            })
        })
        .collect::<Result<Vec<_>>>()?;
    Ok(RuntimePlugin {
...
/// 提取当前 Agent Runtime 尚未识别的 Hook 事件名，按原始顺序去重。
...
fn validate_dependency_names(dependencies: &BTreeMap<String, VersionRequirement>) -> Result<()> {
    for (id, requirement) in dependencies {
        ...
        if requirement.0.trim().is_empty() {
            return Err(PluginError::Invalid(format!("依赖版本要求不能为空：{id}")));
        }
    }
    Ok(())
}
...
fn visit_dependency(
    ...
    let entry = market_plugins
        ...
        .ok_or_else(|| PluginError::Invalid(format!("市场 {marketplace} 中找不到依赖 {id}")))?;
    let manifest = manifests
        .get(&marketplace_name_key(&id.plugin))
        .ok_or_else(|| PluginError::Invalid(format!("没有已解析的插件清单，无法计算依赖：{id}")))?;
...
fn canonical_dependency_id(
    ...
    PluginId::from_components(&entry.name, Some(marketplace))
}
...
fn read_limited(path: &Path) -> Result<Vec<u8>> {
    ...
    if actual_len > MAX_MANIFEST_BYTES || actual_len != opened_metadata.len() {
        return Err(PluginError::Invalid(format!(
            "清单在读取期间发生变化或超过 {} 字节：{}",
            MAX_MANIFEST_BYTES,
            path.display()
...
fn parse_plugin_source(value: Value) -> Result<PluginSource> {
    ...
    let object = value
        .as_object()
        .ok_or_else(|| PluginError::Invalid("插件 source 必须是相对路径字符串或对象".to_owned()))?;
    let source = object_string(object, "source")?;
    match source.as_str() {
                    ...
                    Value::Array(values) => values
                        ...
                        .map(|value| {
                            value.as_str().map(ToOwned::to_owned).ok_or_else(|| {
                                PluginError::Invalid("mcpServers files 必须是字符串数组".to_owned())
                            })
                        })
                        .collect::<Result<Vec<_>>>()?,
                    _ => {
...
fn validate_mcp_reference(file: &str) -> Result<()> {
    if file.starts_with("http://") || file.starts_with("https://") {
        let parsed = url::Url::parse(file)
            .map_err(|_| PluginError::Invalid("MCPB/DXT URL 格式无效".to_owned()))?;
...
fn object_string(object: &Map<String, Value>, key: &str) -> Result<String> {
    object
        ...
        .ok_or_else(|| PluginError::Invalid(format!("source 缺少非空 string {key}")))
}
...
fn normalized_identifier(value: &str, label: &str) -> Result<String> {
    ...
    if !valid {
        return Err(PluginError::Invalid(format!(
            "{label} 必须是 1 到 128 字节的稳定 ASCII 标识符"
        )));
    }
    Ok(value.to_owned())
}
...
fn non_empty<'a>(value: &'a str, label: &str) -> Result<&'a str> {
    if value.trim().is_empty() {
        return Err(PluginError::Invalid(format!("{label} 不能为空")));
    }
    Ok(value)
}
...
/// 校验 KeenCode marketplace/plugin source 使用的固定 40 位提交 SHA。
...
fn package_spec(package: &str, version: Option<&str>) -> Result<String> {
    ...
    if package.contains(char::is_whitespace) {
        return Err(PluginError::Invalid(format!("包名不能包含空白：{package}")));
    }
    match version {
...
/// 返回安全的相对路径；允许 KeenCode 市场惯用的 `./plugin`，拒绝 `..` 和绝对路径。
...
fn validate_component_paths(paths: &[String], label: &str) -> Result<()> {
    ...
    for path in paths {
        ...
        if !unique.insert(path) {
            return Err(PluginError::Invalid(format!("{label} 包含重复路径")));
        }
    }
    Ok(())
}
...
fn validate_user_config_value(
    ...
    let values = if definition.multiple {
        value.as_array().ok_or_else(|| {
            PluginError::Invalid(format!("userConfig {name} 启用 multiple 时值必须为数组"))
        })?
    } else {
...
/// 规范化并确认插件运行时使用的项目根是现存目录。
fn canonical_project_directory(project_dir: &Path) -> Result<PathBuf> {
...
/// 扫描清单显式声明的组件；未声明时遵循 KeenCode 的默认目录约定。
...
/// 递归展开 KeenCode Hook 的包装对象、路径数组和 hooks/hooks.json 文件。
fn merge_hook_declaration(
    ...
    match declaration {
        ...
        _ => Err(PluginError::Invalid(
            "KeenCode Hooks 声明必须是对象、路径或数组".to_owned(),
        )),
    }
}
...
/// 将配置名转换为可由 `${KEENCODE_PLUGIN_*}` 引用的稳定变量名。
...
/// 把插件声明的 DXT/MCPB `manifest.user_config` 合并为 KeenCode 插件 userConfig。
///
/// KeenCode 允许插件通过 MCP bundle 声明配置；这些字段必须进入同一个
...
fn merge_mcp_bundle_user_config(root: &Path, manifest: &mut PluginManifest) -> Result<()> {
    ...
    for declaration in declarations {
        ...
        let object = bundle_manifest
            .as_object()
            .ok_or_else(|| PluginError::Invalid("MCPB/DXT manifest 顶层必须是对象".to_owned()))?;
        ...
        for (name, definition) in schema {
            ...
            let parsed: UserConfigDefinition =
                serde_json::from_value(definition.clone()).map_err(|error| {
                    PluginError::Invalid(format!("MCPB/DXT userConfig {name} 定义无效：{error}"))
                })?;
...
fn read_mcp_bundle_reader<R: Read>(reader: &mut R) -> Result<Vec<u8>> {
    ...
    reader
        ...
        .read_to_end(&mut bytes)
        .map_err(|error| PluginError::Invalid(format!("读取 MCPB/DXT 失败：{error}")))?;
...
fn read_mcp_bundle_file(path: &Path) -> Result<Vec<u8>> {
    ...
    // 使用 no-follow 句柄关闭“检查后、打开前”被替换成符号链接的竞态。
    let mut file = crate::storage::open_readonly_regular_file(path)?;
...
fn materialize_mcp_bundle(root: &Path, declaration: &str) -> Result<(PathBuf, Value)> {
    ...
    let bytes = if declaration.starts_with("http://") || declaration.starts_with("https://") {
        let cache_path = archives_root.join(
            mcp_bundle_url_cache_path(&root, declaration)
                .file_name()
                .ok_or_else(|| PluginError::Invalid("MCPB/DXT URL 缓存文件名无效".to_owned()))?,
        );
        if let Some(bytes) = read_mcp_bundle_cache(&cache_path)? {
            ...
            let client = reqwest::blocking::Client::builder()
                ...
                .build()
                .map_err(|error| {
                    PluginError::Invalid(format!("MCPB HTTP 客户端创建失败：{error}"))
                })?;
            let response = client
                .get(declaration)
                .header(reqwest::header::USER_AGENT, "KeenCode-Plugin/1")
                .send()
                .map_err(|error| PluginError::Invalid(format!("下载 MCPB/DXT 失败：{error}")))?
                .error_for_status()
                .map_err(|error| {
                    PluginError::Invalid(format!("下载 MCPB/DXT 返回错误：{error}"))
                })?;
            let bytes = match read_http_response_limited(response, MAX_MCPB_BYTES) {
                ...
                Err(HttpResponseReadError::Read(error)) => {
                    return Err(PluginError::Invalid(format!("读取 MCPB/DXT 失败：{error}")));
                }
            };
            ...
            crate::storage::atomic_write_private(&cache_path, &bytes).map_err(|error| {
                PluginError::Invalid(format!("写入 MCPB/DXT 本地缓存失败：{}", error))
            })?;
            bytes
        }
    } else {
    ...
    let _extraction_guard = MCPB_EXTRACTION_LOCK
        .lock()
        .map_err(|_| PluginError::Invalid("MCPB/DXT 解包缓存锁已损坏".to_owned()))?;
    ...
    let mut archive = zip::ZipArchive::new(Cursor::new(bytes))
        .map_err(|error| PluginError::Invalid(format!("MCPB/DXT ZIP 无效：{error}")))?;
    if archive.len() > MAX_MCPB_ENTRIES {
        return Err(PluginError::Invalid("MCPB/DXT 文件数量超过限制".to_owned()));
    }
    ...
    for index in 0..archive.len() {
        let mut entry = archive
            .by_index(index)
            .map_err(|error| PluginError::Invalid(format!("读取 MCPB/DXT 条目失败：{error}")))?;
        let relative = entry
            .enclosed_name()
            .ok_or_else(|| {
                PluginError::Invalid(format!("MCPB/DXT 条目路径越界：{}", entry.name()))
            })?
            .to_path_buf();
        ...
        extracted_bytes = extracted_bytes
            ...
            .ok_or_else(|| PluginError::Invalid("MCPB/DXT 解包大小溢出".to_owned()))?;
...
/// 将 DXT/MCPB `manifest.json` 的 server 描述转换成 KeenCode MCP 配置。
fn mcp_bundle_servers(extracted: &Path, manifest: Value) -> Result<BTreeMap<String, Value>> {
    ...
    let server = object
        .get("server")
        .and_then(Value::as_object)
        .ok_or_else(|| PluginError::Invalid("MCPB/DXT manifest 缺少 server 配置".to_owned()))?;
    let name = object
        .get("name")
        .and_then(Value::as_str)
    ...
    if config.is_null() {
        let entry_point = server
            ...
            .ok_or_else(|| PluginError::Invalid("MCPB/DXT server 缺少 entry_point".to_owned()))?;
...
fn parse_mcp_servers_value(value: Value, path: &Path) -> Result<BTreeMap<String, Value>> {
    let object = value.as_object().ok_or_else(|| {
        PluginError::Invalid(format!("MCP 配置顶层必须是对象：{}", path.display()))
    })?;
    let servers = object
        .get("mcpServers")
        ...
        Value::Array(values) => {
            for value in values {
                let mut entry = value.as_object().cloned().ok_or_else(|| {
                    PluginError::Invalid("mcpServers 数组项必须是对象".to_owned())
                })?;
                let name = entry
                    .remove("name")
                    .and_then(|value| value.as_str().map(ToOwned::to_owned))
                    .ok_or_else(|| PluginError::Invalid("mcpServers 数组项缺少 name".to_owned()))?;
                let config = entry.remove("config").unwrap_or(Value::Object(entry));
...
fn insert_mcp_server(
    ...
    if servers.insert(name.clone(), config).is_some() {
        return Err(PluginError::Invalid(format!("MCP Server 名称重复：{name}")));
    }
    Ok(())
}

/// 将 KeenCode MCP Server 的 `type` 判别字段归一为 command/url 传输字段。
fn normalize_mcp_server_value(value: Value) -> Result<Value> {
    let mut object = value
        .as_object()
        .cloned()
        .ok_or_else(|| PluginError::Invalid("MCP Server 配置必须是对象".to_owned()))?;
        ...
        .and_then(|value| value.as_str().map(ToOwned::to_owned))
    {
        match kind.as_str() {
            ...
            "sse" | "websocket" | "sdk" | "keencodeai-proxy" => {
                return Err(PluginError::Invalid(format!(
            ...
            other => {
                return Err(PluginError::Invalid(format!(
                    "未知 KeenCode MCP transport type={other}"
                )));
            }
        }
    }
...
fn fingerprint_plugin_tree(root: &Path, current: &Path, hasher: &mut Sha256) -> Result<()> {
    ...
    for entry in entries {
        ...
        let relative = path
            .strip_prefix(root)
            .map_err(|error| PluginError::Invalid(format!("插件路径无法生成摘要：{error}")))?;
        if relative
            .components()
        ...
        if metadata.file_type().is_symlink() {
            ...
            hasher.update(
                target
                    .strip_prefix(root)
                    .map_err(|error| {
                        PluginError::Invalid(format!("插件符号链接目标无效：{error}"))
                    })?
                    .to_string_lossy()
                    .as_bytes(),
            );
...
fn copy_plugin_tree(source: &Path, destination: &Path) -> Result<()> {
    ...
    let parent = destination
        .parent()
        .ok_or_else(|| PluginError::Invalid("插件缓存目标缺少父目录".to_owned()))?;
    let cache_root = parent
        .parent()
        .and_then(Path::parent)
        .ok_or_else(|| PluginError::Invalid("插件缓存目标缺少缓存根目录".to_owned()))?;
    ...
    for plugin in &previous.plugins {
            ...
            && error.kind() != io::ErrorKind::NotFound
        {
            tracing::warn!(
                path = %path.display(),
                %error,
                "清理未引用的 KeenCode 插件缓存失败"
            );
        }
    }
}

/// 判断路径是否严格匹配 `<cache>/<marketplace>/<plugin>/<fingerprint>` 布局。
...
fn validate_state(storage: &PluginStorage, state: &PluginState) -> Result<()> {
    ...
    for plugin in &state.plugins {
        ...
        if !ids.insert(key) {
            return Err(PluginError::Invalid(format!("插件状态包含重复 ID：{id}")));
        }
        ...
        let fingerprint = plugin
            ...
            .ok_or_else(|| PluginError::Invalid(format!("安装路径缺少内容指纹：{id}")))?;
        if !is_content_fingerprint(fingerprint) {
            return Err(PluginError::Invalid(format!("安装路径内容指纹无效：{id}")));
        }
        ...
        let relative = plugin
            ...
            .map_err(|_| PluginError::Invalid(format!("安装路径不在当前缓存根目录内：{id}")))?;
        let mut components = relative.components();
        if !matches!(
            (
                components.next(),
        ...
        let metadata = fs::symlink_metadata(&plugin.install_path)
            .map_err(|error| PluginError::Invalid(format!("插件缓存目录不可用：{id}：{error}")))?;
        if metadata.file_type().is_symlink() || !metadata.is_dir() {
            return Err(PluginError::Invalid(format!(
...
fn require_marketplace_id(id: &PluginId) -> Result<PluginId> {
    let marketplace = id
        .marketplace
        .as_deref()
        .ok_or_else(|| PluginError::Invalid(format!("插件 ID 缺少市场命名空间：{id}")))?;
