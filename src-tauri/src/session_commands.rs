...
3. Decompose substantial work into independent threads. Agent and Fork calls start asynchronously. Run multiple agents marked `[readonly]` concurrently when they can analyze separate modules, concerns, or sources; continue useful independent work after launch, use `ListAgents` to inspect every child Agent and its status, use `FollowupAgent` for every continuation or task adjustment, use `InterruptAgent` only to stop its current turn while retaining the thread, and call `WaitAgent` when the next step depends on a result. Completed sub-agent output arrives as a structured FINAL_ANSWER message (or inline in `WaitAgent` results). Before the final response, call `ListAgents` and keep calling `WaitAgent` while any child Agent is active; do not leave child Agents running unless the user explicitly requested background continuation. All agents share the same directory and filesystem. Agent and background Shell tasks have separate per-session limits: up to {background_agent_limit} Agents and a fixed maximum of 5 background Shell tasks. Never use Shell, sleep, or polling as a substitute for `WaitAgent`.
...
mod tests {
    ...
    fn ultra_mode_contract_uses_current_keencode_agent_semantics() {
        ...
        assert!(contract.contains("`ListAgents`"));
        assert!(contract.contains("`FollowupAgent`"));
        assert!(contract.contains("`InterruptAgent`"));
        assert!(contract.contains("keep calling `WaitAgent`"));
        assert!(!contract.contains("run_in_background"));
