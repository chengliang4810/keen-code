...
impl SystemSecretStore {
    ...
    fn entry(key: &str) -> Result<Entry> {
        Entry::new(KEYRING_SERVICE, key)
            .map_err(|_| PluginError::Invalid("无法访问系统密钥库".to_owned()))
    }

    /// 将密钥库错误转换为不包含服务名、键名或敏感值的插件错误。
    fn keyring_error(operation: &str) -> PluginError {
        PluginError::Invalid(format!("系统密钥库{operation}失败"))
    }
}
...
impl SecretStore for SystemSecretStore {
    ...
    fn set_json(&mut self, key: &str, value: &Value) -> Result<()> {
        let password = serde_json::to_string(value)
            .map_err(|_| PluginError::Invalid("敏感插件配置 JSON 序列化失败".to_owned()))?;
    ...
    fn get_json(&self, key: &str) -> Result<Option<Value>> {
        ...
        serde_json::from_str(&password)
            .map(Some)
            .map_err(|_| PluginError::Invalid("系统密钥库中的敏感插件配置 JSON 无效".to_owned()))
    }
