...
impl fmt::Display for PluginError {
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Io(error) => write!(formatter, "KeenCode 插件文件操作失败：{error}"),
            Self::Json(error) => write!(formatter, "KeenCode 插件 JSON 格式无效：{error}"),
            Self::Invalid(message) => write!(formatter, "KeenCode 插件配置无效：{message}"),
            ...
            Self::MissingVariable(name) => write!(formatter, "KeenCode 插件缺少变量值：{name}"),
        }
    }
}
...
/// 插件在市场中的稳定身份；未带市场的形式只允许用于用户输入解析。
...
impl<'de> Deserialize<'de> for PluginId {
    /// 读取当前插件契约支持的 `plugin@marketplace` 字符串或严格对象表示。
        ...
        D: Deserializer<'de>,
    {
        ...
        /// 当前插件 ID JSON 允许的两种等价表示。
        #[derive(Deserialize)]
        #[serde(untagged)]
        enum PluginIdRepresentation {
            /// `plugin` 或 `plugin@marketplace` 字符串。
            String(String),
            /// 带独立 plugin 与 marketplace 字段的对象。
            Object(PluginIdFields),
        }

        match PluginIdRepresentation::deserialize(deserializer)? {
            PluginIdRepresentation::String(value) => {
                Self::parse(&value).map_err(serde::de::Error::custom)
            }
            PluginIdRepresentation::Object(fields) => {
                Self::from_components(&fields.plugin, fields.marketplace.as_deref())
...
impl PluginId {
    /// 解析 `plugin` 或 `plugin@marketplace`，拒绝多余分隔符和非稳定标识符。
    pub fn parse(value: &str) -> Result<Self> {
        let value = value.trim();
        if value.is_empty() {
            return Err(PluginError::Invalid("插件 ID 不能为空".to_owned()));
        }
    ...
    /// 从独立字段构造已校验身份。
    pub fn from_components(plugin: &str, marketplace: Option<&str>) -> Result<Self> {
        Ok(Self {
    ...
    /// 返回绑定到指定市场的身份；已有不同市场时拒绝静默改写。
    pub fn in_marketplace(&self, marketplace: &str) -> Result<Self> {
        let marketplace = normalized_identifier(marketplace, "市场名称")?;
        if self
            .marketplace
            .as_deref()
            .is_some_and(|current| !current.eq_ignore_ascii_case(&marketplace))
        {
            return Err(PluginError::Invalid("插件 ID 与目标市场不一致".to_owned()));
        }
        Self::from_components(&self.plugin, Some(&marketplace))
    }

    /// 返回运行时组件共用的市场级命名空间，拒绝尚未绑定市场的用户输入 ID。
    pub fn runtime_namespace(&self) -> Result<String> {
        let marketplace = self.marketplace.as_deref().ok_or_else(|| {
            PluginError::Invalid("运行时插件必须使用 plugin@marketplace".to_owned())
        })?;
        Ok(format!("plugin:{marketplace}:{}", self.plugin))
    }
}

impl fmt::Display for PluginId {
    /// 输出稳定的 `plugin@marketplace` 或未绑定市场的 `plugin`。
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(&self.plugin)?;
        if let Some(marketplace) = &self.marketplace {
            formatter.write_str("@")?;
            formatter.write_str(marketplace)?;
        }
        Ok(())
    }
}
...
pub struct PluginManifest {
    ...
    pub name: String,
    /// 插件版本；KeenCode 允许省略，缓存会使用受控的 `unversioned` 段。
    #[serde(default)]
    pub version: Option<String>,
    /// 插件用途说明。
    #[serde(default)]
...
/// 插件仓库字段，接受 KeenCode 清单使用的字符串或对象形式。
...
/// `mcpServers` 的归一化声明；KeenCode 清单可内联 Server、提供数组，或引用相对 JSON 文件。
#[derive(Clone, Debug, Default, Serialize)]
...
/// KeenCode 插件 LSP Server 声明。
#[derive(Clone, Debug, Deserialize, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct PluginLspServer {
    /// 插件内唯一的 Server 名称；运行时会加上 `plugin:<marketplace>:<plugin>:` 命名空间。
    pub name: String,
    /// 直接交给 LSP 子进程的可执行命令，支持 KeenCode 插件变量插值。
    pub command: String,
    /// 直接交给 LSP 子进程的参数，支持 KeenCode 插件变量插值。
    #[serde(default)]
    pub args: Vec<String>,
    /// 传递给 LSP 子进程的环境变量，支持 KeenCode 插件变量插值。
    #[serde(default)]
    ...
    /// 未识别字段原样保存，避免新版 KeenCode 插件字段导致整个市场不可用。
    #[serde(default, flatten)]
    pub extra: BTreeMap<String, Value>,
}
...
pub enum PluginSource {
    ...
    /// HTTP(S) Git 仓库 URL；KeenCode 将 `url` source 解释为 Git 仓库。
    Url {
        /// 可克隆的 Git 仓库地址。
        url: String,
        /// 可选 branch、tag 或 commit。
...
impl PluginStorage {
    /// 依据应用数据根目录构造当前唯一的 KeenCode 插件布局。
    pub fn under(data_root: impl Into<PathBuf>) -> Self {
        let root = data_root.into().join("plugins");
        Self {
            root: root.clone(),
            cache_root: root.join("cache"),
            state_path: root.join("state.json"),
            secret_namespace: "keencode.plugin".to_owned(),
        }
    }
    ...
    pub fn versioned_path(&self, id: &PluginId, version: &str) -> Result<PathBuf> {
        let marketplace = id.marketplace.as_deref().ok_or_else(|| {
            PluginError::Invalid("版本化缓存必须使用 plugin@marketplace".to_owned())
        })?;
        Ok(self
            .cache_root
    ...
    /// 校验受控目录层级，不跟随 `.keencode/plugins`、`cache` 或状态路径中的
    ...
    pub fn secret_key_at(&self, id: &PluginId, field: &str, generation: u64) -> Result<String> {
        let id =
            id.in_marketplace(id.marketplace.as_deref().ok_or_else(|| {
                PluginError::Invalid("敏感配置必须使用带市场的插件 ID".to_owned())
            })?)?;
        let marketplace =
        ...
        digest.update(b"keencode.plugin/secret-key/v1\0");
