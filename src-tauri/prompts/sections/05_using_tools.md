...
- Start with a focused read-only query and broaden only as evidence requires. Parallelize independent calls; sequence dependent operations and conflicting writes. Emit independent Read, Glob, and Grep calls together in one response, since those run concurrently; Bash and PowerShell are serialized, so do not expect shell commands to run in parallel.
