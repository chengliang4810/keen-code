...
import { isOfficeKind, pathToFileUrl } from "./filePreviewSrc";
...
describe("isOfficeKind", () => {
  it("只识别当前支持的 Office 与 ODF 文档", () => {
    expect(isOfficeKind("docx")).toBe(true);
    expect(isOfficeKind("xlsx")).toBe(true);
    expect(isOfficeKind("pptx")).toBe(true);
    expect(isOfficeKind("odf")).toBe(true);
