...
const msgs: ChatFindMessage[] = [
  { id: "u1", role: "user", content: "Please fix the Runtime reset flow" },
  {
    id: "a1",
    role: "assistant",
    content: "I'll inspect Runtime reset and the runtime provider path.",
  },
  {
    id: "t1",
    role: "tool",
    content: "tool_step|completed|read|Read runtime.rs\n/tmp/runtime.rs",
    toolTitle: "Read runtime.rs",
    marker: "tool_step",
  },
...
describe("searchableTextForMessage", () => {
  it("uses content for user/assistant", () => {
    expect(searchableTextForMessage(msgs[0]!)).toContain("Runtime");
    expect(searchableTextForMessage(msgs[1]!)).toContain("provider path");
  });

  it("prefers toolTitle for tools", () => {
    expect(searchableTextForMessage(msgs[2]!)).toBe("Read runtime.rs");
  });
...
describe("findChatMatches", () => {
  ...
  it("matches case-insensitively across messages", () => {
    const hits = findChatMatches("runtime", msgs);
  ...
  it("tracks per-message occurrence", () => {
    ...
    // "Runtime" + "runtime" in assistant content
...
describe("splitHighlightParts", () => {
  ...
  it("splits multiple case-insensitive hits", () => {
    const parts = splitHighlightParts("Foo bar FOO", "foo");
    ...
    expect(parts.map((p) => p.text).join("")).toBe("Foo bar FOO");
  });
});
