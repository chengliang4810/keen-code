/** 繁體中文（台灣）文案目錄，鍵集合與目前的 `en`、`zh` 界面一致。 */
import type { MessageKey } from "./messages";

export const zhTW: Record<MessageKey, string> = {
  "app.versionFooter": "KeenCode · MIT",

  "tray.show": "顯示 KeenCode",
  "tray.quit": "結束 KeenCode",

  "window.minimize": "最小化",
  "window.maximize": "最大化",
  "window.restore": "還原",
  "window.close": "關閉",

  "sidebar.search": "搜尋",
  "sidebar.skills": "技能",
  "sidebar.plugins": "外掛市集",
  "sidebar.projects": "專案",
  "sidebar.pinned": "置頂",
  "sidebar.addProject": "新增專案",
  "sidebar.projectMoved": "已將「{name}」移至第 {position} 項，共 {total} 項。",
  "sidebar.collapseAllProjects": "收合全部專案",
  "sidebar.sort": "對話排序方式",
  "sidebar.sortByLastUserMessage": "最後使用者訊息時間",
  "sidebar.sortByUpdatedAt": "更新時間",
  "sidebar.noProjects": "還沒有專案。點擊 + 新增本機目錄。",
  "sidebar.pathMissing": "路徑失效",
  "sidebar.newConversation": "新增對話",
  "sidebar.menu": "選單",
  "sidebar.relocateProject": "重新定位資料夾…",
  "sidebar.noChats": "尚無對話",
  "sidebar.showMore": "顯示更多",
  "sidebar.otherSessions": "對話",
  "sidebar.settings": "設定",
  "sidebar.installUpdate": "安裝 {version} 並重新啟動",
  "sidebar.updatePreparing": "正在背景準備 {version}",
  "sidebar.archive": "封存",
  "sidebar.unarchive": "取消封存",
  "sidebar.current": "目前對話",
  "sidebar.archived": "已封存對話",
  "sidebar.noArchivedSessions": "尚無已封存對話",
  "sidebar.sessionWorking": "進行中…",
  "sidebar.sessionCompletedUnread": "已完成，點擊後標記為已查看",
  "sidebar.sessionFailedUnread": "已失敗，點擊查看錯誤",
  "sidebar.needsUserInput": "需要使用者輸入",

  "project.reveal": "在 Finder 中顯示",
  "project.rename": "重新命名專案",
  "project.relocate": "重新定位…",
  "project.relocateOk": "「{name}」已指向 {path}",
  "project.remove": "移除",
  "project.removeTitle": "移除專案",
  "project.removeConfirmDetail":
    "從應用程式清單移除「{name}」？僅刪除專案記錄；磁碟上的資料夾以及相關對話都不會刪除。",
  "project.pathMissing":
    "「{name}」的資料夾已遺失或不是目錄。請重新定位後繼續。",
  "project.pathMissingShort": "資料夾遺失",
  "project.removedMissing": "專案目錄已被刪除，已從清單移除：{name}",
  "project.relocateToSend": "重新定位…",

  "addProject.title": "建立專案",
  "addProject.name": "專案名稱",
  "addProject.namePlaceholder": "專案名稱",
  "addProject.sourceFolder": "現有專案資料夾（選填）",
  "addProject.chooseFolder": "選擇現有專案資料夾",
  "addProject.defaultLocation": "KeenCode 將在 {path} 建立專案",
  "addProject.changeDefaultLocation": "前往設定修改",
  "addProject.nameRequired": "請輸入專案名稱。",
  "addProject.existingSelected": "該目錄已是專案，已選擇「{name}」。",
  "addProject.folderOnly": "請選擇一個資料夾作為專案。",
  "addProject.oneFolderOnly": "每次只能新增一個資料夾。",
  "addProject.submit": "建立專案",
  "addProject.adding": "正在建立…",

  "session.pin": "置頂對話",
  "session.unpin": "取消置頂",
  "session.pinned": "已置頂",
  "session.justNow": "剛剛",
  "session.minutesAgo": "{minutes}分",
  "session.hoursAgo": "{hours}小時",
  "session.daysAgo": "{days}天",
  "session.rename": "重新命名",
  "session.renamePrompt": "重新命名對話",
  "session.renamePlaceholder": "對話標題",
  "session.copyId": "複製對話 ID",
  "session.menu": "對話選單",
  "session.new": "新對話",
  "session.placeholderTitle": "新對話",
  "session.untitled": "未命名",
  "session.empty": "目前對話還沒有訊息。",
  "session.fork": "分叉對話",
  "session.viewTrajectory": "查看軌跡",
  "session.forkTitle": "分叉對話",
  "session.forkTitleOf": "分叉：{name}",
  "session.forkConfirm":
    "基於目前對話複製並建立新對話？原對話保持不變。",
  "session.forkOk": "已分叉 · 已開啟新對話",
  "session.forkFailed": "分叉對話失敗",

  "main.leftPaneShow": "顯示側邊欄",
  "main.leftPaneHide": "隱藏側邊欄",
  "main.resizeLeftPane": "調整側邊欄寬度",
  "main.rightPaneShow": "顯示側邊欄",
  "main.rightPaneHide": "隱藏側邊欄",
  "main.openLocation": "開啟位置",
  "main.openLocationHint": "以上次選擇的應用程式開啟",
  "main.openLocationMenu": "選擇應用程式",
  "main.reconnect": "重新連線 Agent",
  "main.summaryHide": "隱藏摘要",
  "main.summaryShow": "顯示摘要",
  "error.details": "詳情",
  "error.hideDetails": "收合詳情",
  "error.action.reconnect": "重新連線",
  "error.action.retry": "重試",
  "error.action.openAccount": "帳號與金鑰",
  "error.action.openProviders": "服務商",
  "error.action.dismiss": "關閉",
  "error.deck.runtime.problem": "本機 Agent 執行環境無法使用",
  "error.deck.runtime.cause":
    "內建本機 Agent 執行環境無法啟動。請先重新連線；若仍失敗，請重新安裝 KeenCode。",
  "error.deck.auth.problem": "驗證失敗",
  "error.deck.auth.cause":
    "登入過期、API 金鑰無效，或服務商拒絕了請求（401）。",
  "error.deck.network.problem": "網路或模型服務異常",
  "error.deck.network.cause":
    "DNS/逾時、中轉 5xx、base URL 錯誤，或模型 ID 不可用。",
  "error.deck.crash.problem": "Agent 處理程序異常結束",
  "error.deck.crash.cause":
    "Agent 當掉或通訊中斷。對話紀錄仍保留，可重新連線後繼續。",
  "error.deck.quota.problem": "額度不足或觸發限流",
  "error.deck.quota.cause":
    "用量上限、訂閱限流，或服務商回傳 429 / 餘額不足。",
  "error.deck.connect.problem": "無法連線本對話的 Agent",
  "error.deck.connect.cause":
    "目前沒有可用 Agent。執行環境可能無法使用，或自訂供應商設定不完整。",
  "error.deck.limit.problem": "已達 Agent 處理程序上限",
  "error.deck.limit.cause":
    "並行 Agent 過多。請先結束其他對話，或在執行環境中提高上限。",
  "error.deck.timeout.problem": "本輪執行逾時",
  "error.deck.timeout.cause":
    "等待過久已中止。可重試；生圖等長任務可能需要更久。",
  "error.deck.disconnect.problem": "與 Agent 的連線已中斷",
  "error.deck.stall.problem": "輸出似乎卡住了",
  "error.deck.stall.cause":
    "約 {seconds} 秒沒有新的內容或工具進度。可取消本輪或繼續等待。",
  "error.deck.disconnect.cause":
    "傳輸通道在回合中關閉。請重新連線後再傳送。",
  "error.deck.generic.problem": "出了點問題",
  "error.deck.generic.cause": "請重新連線後再試；供應商錯誤可在模型設定中檢查目前設定。",
  "main.startTitle": "今天想從哪裡開始？",
  "main.greeting.morningEarly": "早安，新的一天準備開始吧",
  "main.greeting.morning": "早上好，今天想讓我幫你做什麼？",
  "main.greeting.noon": "午安，準備好繼續了嗎？",
  "main.greeting.afternoon": "下午好，接下來的工作交給我吧",
  "main.greeting.evening": "晚上好，今天辛苦了",
  "main.greeting.lateNight": "夜深了，也別忘了照顧好自己",
  "main.startHint": "輸入訊息後傳送，本機執行環境會按需連線。",
  "main.noProject": "未選專案",
  "resources.title": "資源",
  "resources.files": "檔案",
  "resources.empty": "尚無檔案",
  "resources.loading": "載入中…",
  "resources.htmlEmpty": "沒有可顯示的 HTML 內容。",
  "resources.truncated": "預覽已截斷",
  "resources.binary": "此格式尚無內嵌預覽",
  "resources.filterPh": "篩選檔案…",
  "resources.refresh": "重新整理",
  "resources.openFailed": "無法開啟檔案",
  "resources.openDefault": "以系統預設應用程式開啟",
  "resources.revealFolder": "在檔案管理員中顯示",
  "resources.needProject": "新增或選擇專案後，即可在此瀏覽檔案。",
  "resources.emptyPreview": "尚未開啟檔案",
  "resources.emptyPreviewHint": "從右側檔案樹選擇檔案進行預覽。",
  "resources.tabClose": "關閉分頁",
  "resources.newTab": "新增分頁",
  "resources.openTab": "開啟分頁",
  "resources.openTabHint": "選擇要在側邊面板中開啟的分頁。",
  "resources.tabCloseOthers": "關閉其他分頁",
  "resources.tabCloseRight": "關閉右側分頁",
  "resources.tabCloseLeft": "關閉左側分頁",
  "resources.tabCloseAll": "關閉全部分頁",
  "resources.collapseTree": "收合檔案樹",
  "resources.expandTree": "展開檔案樹",
  "resources.resizeTree": "調整檔案樹寬度",
  "resources.save": "儲存",
  "resources.saving": "儲存中…",
  "resources.revert": "還原",
  "resources.unsaved": "有未儲存的修改",
  "resources.editMode": "編輯",
  "resources.previewMode": "預覽",
  "resources.editorAria": "編輯 {name}",
  "resources.editorToolbar": "檔案編輯工具列",
  "resources.saveFailed": "儲存失敗",
  "resources.saveNoPath": "沒有可儲存的路徑",
  "resources.conflictTitle": "磁碟上的檔案已變更",
  "resources.conflictBody":
    "編輯期間，Agent 或其他程式改寫了此檔案。可重新載入磁碟版本，或用目前緩衝區覆寫。",
  "resources.conflictReload": "重新載入",
  "resources.conflictOverwrite": "覆寫",
  "resources.discardTitle": "放棄修改？",
  "resources.discardBody": "此分頁有未儲存的修改，確定關閉？",
  "resources.discardConfirm": "放棄",
  "terminal.title": "終端機",
  "terminal.tabsAria": "終端機分頁",
  "terminal.new": "終端機",
  "terminal.tabName": "終端機 {number}",
  "terminal.exited": "已結束",
  "terminal.empty": "點擊 + 新增終端機",
  "terminal.needProject": "選擇專案後即可新增終端機",
  "terminal.startFailed": "無法啟動終端機：{error}",

  // 軌跡台帳（右側停駐欄的工作階段記錄檢視）
  "trajectory.title": "軌跡",
  "trajectory.searchPh": "搜尋軌跡",
  "trajectory.collapseTurns": "摺疊全部輪次",
  "trajectory.expandTurns": "展開全部輪次",
  "trajectory.refresh": "重新整理",
  "trajectory.retry": "重試",
  "trajectory.loadFailed": "軌跡載入失敗",
  "trajectory.noSession": "暫無工作階段",
  "trajectory.empty": "暫無軌跡記錄",
  "trajectory.emptyHint": "工作階段執行後會出現記錄。",
  "trajectory.turn": "第 {n} 輪",
  "trajectory.turnSummary": "{records} 筆記錄 · {tools} 個工具",
  "trajectory.stats": "{records} 筆記錄 · {tools} 個工具 · {turns} 輪",
  "trajectory.footerTokens": "輸入 Token {input} · 快取 {cache}",
  "trajectory.noOutput": "無輸出",
  "trajectory.kind.user": "使用者",
  "trajectory.kind.assistant": "回覆",
  "trajectory.kind.thinking": "思考",
  "trajectory.kind.tool": "工具",
  "trajectory.kind.subagent": "子代理",
  "trajectory.kind.compacted": "壓縮",
  "trajectory.kind.error": "錯誤",
  "trajectory.kind.cancelled": "已取消",
  "trajectory.detail.input": "輸入",
  "trajectory.detail.output": "輸出",
  "trajectory.detail.thinking": "思考",
  "trajectory.detail.metrics": "指標",
  "trajectory.detail.attachments": "附件",
  "trajectory.detail.toolKind": "工具類型",
  "trajectory.detail.path": "路徑",
  "trajectory.detail.startedAt": "開始時間",
  "trajectory.detail.duration": "耗時",
  "trajectory.detail.toolCalls": "工具呼叫",
  "trajectory.detail.tokens": "Token",
  "trajectory.metrics.inputTokens": "輸入 Token {value}",
  "trajectory.metrics.reasoningTokens": "推理 Token {value}",
  "trajectory.metrics.cacheRead": "快取讀取 {value}",
  "trajectory.metrics.cacheCreation": "快取建立 {value}",
  "trajectory.compact.trigger": "觸發方式",
  "changes.title": "變更",
  "changes.files": "檔案",
  "changes.empty": "本工作階段尚無檔案變更",
  "changes.emptyHint": "當 Agent 寫入或編輯檔案時，會在此列出供審閱。",
  "changes.showPanel": "顯示工作階段變更",
  "changes.hidePanel": "隱藏工作階段變更",
  "changes.reveal": "在檔案管理員中顯示",
  "changes.openFile": "查看檔案",
  "changes.copyPath": "複製路徑",
  "changes.pathCopied": "路徑已複製",
  "changes.loadingDiff": "正在載入 diff…",
  "changes.largeDiffFallback":
    "Diff 較大，已切換為輕量文字預覽；最多顯示前 {lines} 行或約 {size} KiB。",
  "changes.noDiff": "暫無此檔案的統一 diff",
  "changes.noDiffHint":
    "無法取得修改前內容。可在編輯器中開啟該檔案，或在檔案管理員中顯示。",
  "changes.afterOnly": "目前檔案內容（無修改前快照）",
  "changes.sourcePayload": "來自工具參數",
  "changes.sourceGit": "來自 git",
  "changes.sourceHead": "來自 git HEAD + 工作區",
  "changes.status.completed": "完成",
  "changes.status.failed": "失敗",
  "changes.status.in_progress": "進行中",
  "changes.status.pending": "等待中",
  "changes.section.session": "工作階段",
  "changes.section.workspace": "工作區",

  "changes.workspace.empty": "工作區無 git 變更",
  "changes.workspace.emptyHint":
    "當 git 可用時，專案中已修改、新增與未追蹤檔案會顯示於此。",
  "changes.workspace.unavailable": "無法取得 git 狀態",
  "changes.workspace.noRepo": "目前專案不是 git 倉庫",
  "changes.workspace.noGit": "系統 PATH 中找不到 git",
  "changes.workspace.refresh": "重新整理工作區狀態",
  "changes.workspace.branch": "分支 {branch}",
  "changes.workspace.loading": "正在載入工作區狀態…",
  "changes.workspace.directoryTruncated":
    "未追蹤目錄檔案較多，僅顯示 Git 篩選後的前 {count} 項。",
  "changes.workspace.kind.modified": "已修改",
  "changes.workspace.kind.untracked": "未追蹤",
  "prov.chatOutputTokenField": "Chat 輸出預算欄位",
  "changes.workspace.kind.unknown": "已變更",
"media.loadError": "應用程式內無法載入此媒體。",
  "media.openExternal": "以系統播放器開啟",

  "office.loading": "正在轉譯文件…",
  "office.renderFailed": "無法在應用程式內轉譯此文件",
  "office.openExternal": "以外部應用程式開啟",
  "office.unsupported": "此格式尚無應用程式內預覽",
  "office.pptxLimited":
    "PowerPoint / ODF 目前僅支援有限預覽。完整版面請以外部應用程式開啟。",
  "office.xlsxExternalOnly":
    "基於安全考量已停用表格內預覽。完整內容請以外部應用程式開啟。",

  "composer.placeholder": "隨心輸入",
  "composer.inputLabel": "任務描述",
  "composer.add": "新增",
  "composer.addProject": "新增專案",
  "composer.worktrees": "Git worktrees",
  "composer.worktreesEmpty": "沒有關聯的 worktree",
  "composer.worktreesUnavailable": "無法列出 worktree（需要 git 倉庫）",
  "composer.worktreesLoading": "正在載入 worktree…",
  "composer.worktreeCurrent": "目前",
  "composer.worktreeMain": "主目錄",
  "composer.worktreeDetached": "游離 HEAD",
  "composer.worktreeTip": "切換 Git worktree / 分支",
  "composer.worktreeSwitched": "已切換到 worktree {name}（{branch}）",
  "composer.worktreeNew": "新建 worktree",
  "composer.worktreeNewChat": "新建 worktree 並開聊",
  "composer.worktreeNewTitle": "新建 Git worktree",
  "composer.worktreeNewChatTitle": "新建 worktree 並開聊",
  "composer.worktreeNewHint":
    "在主 worktree 旁建立同級目錄，並檢出一條新分支。",
  "composer.worktreeNewChatHint":
    "建立同級 worktree，並以該目錄為專案開始新對話。",
  "composer.worktreeName": "名稱",
  "composer.worktreeNamePlaceholder": "feat-login",
  "composer.worktreeRef": "起始點（可選）",
  "composer.worktreeRefPlaceholder": "main、origin/main 或 commit",
  "composer.worktreePathPreview": "路徑：{path}",
  "composer.worktreeCreate": "建立",
  "composer.worktreeCreateChat": "建立並開聊",
  "composer.worktreeCreating": "正在建立…",
  "composer.worktreeCreated": "已建立 worktree {name}（{branch}）",
  "composer.worktreeCreatedChat":
    "已建立 worktree {name}（{branch}）· 已開新對話",
  "composer.worktreeNameRequired": "請輸入 worktree 名稱",
  "composer.worktreeNameInvalid":
    "僅允許字母、數字、'.'、'_'、'-'（不能含空格或斜線）",
  "composer.addFiles": "上傳檔案",
  "composer.skills": "技能",
  "composer.skillsEmpty": "未發現可呼叫的技能",
  "composer.skillsLoading": "正在載入技能…",
  "composer.addFilesHint": "附加檔案或資料夾到下一則訊息",
  "composer.attachPasteFailed": "無法附加貼上的檔案",
  "composer.attachSaved": "已附加 {name}",
  "composer.sendNeedsReadyAttachments": "請等待附件上傳完成",
  "composer.send": "傳送",
  "composer.sendNeedsModel": "請先新增模型",
  "composer.sendNeedsContent": "請輸入任務或新增附件",
  "composer.sendUnavailable": "對話忙碌時暫時無法傳送",
  "composer.queue": "入佇列 — 本輪結束後傳送（目前對話）",
  "composer.queued": "已入目前對話佇列 — 留在此對話且本輪結束後自動傳送",
  "composer.queueSendFailed":
    "未真正送出，仍在此對話佇列。請留在此對話或稍後回來續發。",
  "composer.queueDroppedOldest": "佇列已滿（{max}）— 已丟棄最舊 {n} 則",
  "composer.queueCount": "此對話佇列 {n} 則",
  "composer.queueHold": "自動傳送已暫停 — 點重試或再傳送以繼續",
  "composer.queueHoldRetry": "重試",
  "composer.queueClear": "清空",
  "composer.queueRemove": "從佇列移除",
  "composer.queueDrag": "拖曳調整順序",
  "composer.queueSendNow": "立即傳送",
  "composer.queueSteer": "引導",
  "composer.queueSteering": "引導中…",
  "composer.queueSteered": "已引導目前執行中的任務",
  "composer.queueSteerNotRunning": "任務已停止執行，訊息仍保留在佇列中",
  "composer.queueEmptyPreview": "（附件）",
  "composer.queueFilesCount": "{n} 個檔案",
  "composer.stop": "停止",
  "composer.pickProject": "選擇專案",
  "composer.projectBound": "對話已繫結至「{name}」",
  "composer.projectCleared": "已取消對話的專案繫結",
  "composer.model": "模型",
  "composer.modelVision": "視覺",
  "composer.addModel": "新增模型",
  "composer.manageModels": "管理模型",
  "composer.effort": "思考程度",
  "composer.ultra": "Ultra",
  "composer.ultraDescription": "主動將複雜工作委派給專用 Agent",
  "composer.planMode": "計劃",
  "composer.planModeToggle": "切換計劃模式（唯讀調研與規劃）",
  "composer.planModeOff": "關閉計劃模式",
  "composer.reasoningSupported": "支援",
  "composer.reasoningUnsupported": "不支援",
  "composer.attachRemove": "移除附件",
  "composer.attachCount": "已附加 {n} 項",
  "composer.attachRetry": "重試上傳",
  "composer.attachUploading": "正在上傳",
  "composer.attachFailed": "上傳失敗",
  "composer.mentionEmpty": "沒有相符的引用",
  "composer.mentionLoading": "正在載入外掛引用…",
  "composer.projectAdded": "已新增專案「{name}」",
  "attach.open": "開啟",
  "attach.reveal": "在 Finder 中顯示",
  "attach.copyPath": "複製路徑",
  "attach.copyImage": "複製圖片",
  "attach.addToComposer": "新增到輸入框",
  "attach.droppedNone": "無法讀取這些路徑，請從 Finder 再試一次。",
  "attach.details": "詳情",
  "attach.detailsTitle": "詳情",
  "attach.detailsName": "名稱",
  "attach.detailsType": "類型",
  "attach.detailsPath": "路徑",
  "attach.detailsResolved": "原始路徑",
  "attach.detailsStatus": "狀態",
  "attach.detailsOk": "可用",
  "attach.detailsClose": "關閉",
  "attach.typeFile": "檔案",
  "attach.typeUrl": "連結",
  "attach.typeDir": "資料夾",
  "image.view": "檢視大圖",
  "image.copy": "複製圖片",
  "image.next": "下一張",
  "image.prev": "上一張",
  "image.close": "關閉",
  "image.zoomIn": "放大",
  "image.zoomOut": "縮小",
  "video.loadError": "影片載入失敗",
  "resources.openInPanel": "在資源面板開啟",
  "resources.openExternal": "以系統應用程式開啟",
  "resources.browserReload": "重新整理",
  "resources.browserBack": "上一頁",
  "resources.browserForward": "下一頁",
  "resources.browserAddress": "網址",
  "resources.browserAddressPlaceholder": "輸入網址或本機檔案路徑",
  "resources.browserBadAddress": "請輸入 http(s) 網址或本機檔案路徑。",
  "resources.browserDesktopOnly": "內建瀏覽器在桌面應用程式中執行。",
  "resources.browserNewTab": "新增網頁",
  "resources.web": "網頁",

  "effort.none": "關閉",
  "effort.minimal": "最小",
  "effort.high": "高",
  "effort.medium": "中",
  "effort.low": "低",
  "effort.xhigh": "極高",
  "effort.max": "最大",

  "search.placeholder": "搜尋對話或專案…",
  "search.chats": "對話",
  "search.noMatches": "無相符結果",
  "search.newChat": "新增對話",

  "chatFind.placeholder": "在對話中尋找…",
  "chatFind.prev": "上一個相符",
  "chatFind.next": "下一個相符",
  "chatFind.close": "關閉尋找",
  "chatFind.count": "{current} / {total}",
  "chatFind.noMatches": "無相符",
  "chatFind.aria": "在對話中尋找",


  "settings.backToApp": "返回應用程式",
  "settings.saveFailed": "無法儲存設定，請重試。",
  "settings.title": "設定",
  "settings.navigation": "設定導覽",
  "settings.skipToContent": "跳到設定內容",
  "settings.group.core": "基礎設定",
  "settings.group.agent": "Agent能力",
  "settings.group.extensions": "擴充",
  "settings.group.archive": "封存對話",
  "settings.group.data": "使用資料",
  "settings.nav.general": "一般",
  "settings.nav.appearance": "外觀",
  "settings.nav.archived": "已封存對話",
  "settings.nav.archiveSettings": "封存設定",
  "settings.nav.account": "模型設定",
  "settings.nav.extensions": "擴充功能",
  "settings.nav.market": "外掛市集",
  "settings.nav.plugins": "外掛",
  "settings.nav.personalization": "個人化",
  "settings.nav.requests": "請求記錄",
  "settings.nav.analytics": "用量統計",
  "settings.nav.observability": "執行階段觀測",
  "settings.nav.about": "關於",
  "settings.general.system": "系統",
  "settings.general.notifications": "通知",
  "settings.interfaceLanguage": "介面語言",
  "settings.interfaceLanguageDesc":
    "控制應用程式介面以及產生本機記憶時使用的語言。",
  "settings.projectDirectory": "預設專案儲存位置",
  "settings.projectDirectoryChoose": "選擇資料夾",
  "settings.projectDirectoryReset": "恢復預設",
  "settings.terminalShell": "整合終端 Shell",
  "settings.terminalShellDesc": "選擇新建終端使用的 Windows Shell。",
  "settings.terminalShellAuto": "自動（推薦）",
  "settings.chromeHardwareAcceleration": "Chrome 硬體加速",
  "settings.chromeHardwareAccelerationDesc":
    "關閉後可規避部分顯示卡或驅動造成的白屏、閃退、渲染異常。修改後需重新啟動應用程式生效。",
  "settings.taskNotifications": "任務通知",
  "settings.taskNotificationsDesc": "任務完成或失敗時傳送桌面通知。",
  "settings.showThinkingProcess": "顯示思考過程",
  "settings.showThinkingProcessDesc":
    "在對話中保留已結束的思考過程內容塊；思考進行中始終即時顯示。",
  "settings.notificationSound": "通知聲音",
  "settings.notificationSoundDesc": "為任務通知播放系統預設提示音。",
  "settings.keepComputerAwake": "保持電腦運作",
  "settings.keepComputerAwakeDesc":
    "KeenCode 執行時阻止系統因閒置進入睡眠，仍可手動睡眠或闔蓋休眠。",
  "settings.closeToTray": "關閉視窗後保留在系統匣",
  "settings.closeToTrayDesc":
    "關閉主視窗時隱藏到系統匣而非結束應用程式，執行中的任務會繼續進行。點擊系統匣圖示可隨時還原視窗。",
  "settings.backgroundAgentLimit": "背景 Agent 並行數量",
  "settings.backgroundAgentLimitDesc":
    "所有對話共用的背景 Agent 並行上限（裝置級；預設 10，上限 999）。背景 Shell 獨立計數並固定最多 5 個。",
  "settings.webServiceUrl": "相容服務基礎 URL",
  "settings.webServiceUrlDesc":
    "透過 Tavily 相容服務搜尋、擷取網頁，留空使用內建服務。搜尋詞與網址會傳送至服務。僅接受無憑據、查詢參數及片段的 HTTP/HTTPS 位址。",
  "settings.webServiceUrlPlaceholder": "https://tavily.claude-code-best.win",
  "settings.webHost.title": "桌面 Web Host",
  "settings.webHost.enabled": "允許瀏覽器存取",
  "settings.webHost.enabledDesc":
    "為遠端工作台開放經過驗證的本機 Web Host。監聽器只有在手動啟動後才會開啟。",
  "settings.webHost.bind": "監聽位址",
  "settings.webHost.bindDesc":
    "僅本機使用 127.0.0.1；手機存取請填寫電腦的具體區域網路 IP。公網位址和 0.0.0.0 會被拒絕。",
  "settings.webHost.port": "監聽連接埠",
  "settings.webHost.portDesc":
    "連接埠修改會在下次啟動 Web Host 時生效；除非連接埠被占用，否則建議保留預設值。",
  "settings.webHost.status": "執行狀態",
  "settings.webHost.statusDesc":
    "Web Host 只繫結設定的位址，不會回傳瀏覽器 Token。",
  "settings.webHost.state.unknown": "無法使用",
  "settings.webHost.state.disabled": "已停用",
  "settings.webHost.state.stopped": "已停止",
  "settings.webHost.state.running": "執行中",
  "settings.webHost.state.stopping": "正在停止",
  "settings.webHost.state.failed": "啟動失敗",
  "settings.webHost.connections": "{count} 個作用中連線",
  "settings.webHost.start": "啟動 Web Host",
  "settings.webHost.starting": "正在啟動…",
  "settings.webHost.stop": "停止 Web Host",
  "settings.webHost.stopping": "正在停止…",
  "settings.webHost.refresh": "重新整理狀態",
  "settings.webHost.token": "Web Host Token",
  "settings.webHost.tokenDesc":
    "Token 儲存在作業系統憑據庫中，不會寫入一般設定、記錄或回應。",
  "settings.webHost.tokenPlaceholder": "輸入至少 8 個字元的 Token",
  "settings.webHost.tokenSave": "儲存 Token",
  "settings.webHost.tokenSaving": "正在儲存…",
  "settings.webHost.tokenSaved": "Token 已儲存",
  "settings.webHost.tokenVersion": "目前驗證版本 {version}",
  "settings.webHost.tokenInvalid": "Token 必須為 8–512 個 UTF-8 位元組，且不能包含空白或控制字元。",
  "settings.webHost.statusError": "無法讀取 Web Host 狀態。",
  "settings.webHost.startError": "無法啟動 Web Host。",
  "settings.webHost.stopError": "無法停止 Web Host。",
  "settings.webHost.tokenError": "無法儲存 Web Host Token。",
  "settings.archive.auto": "自動封存舊對話",
  "settings.archive.autoDesc": "定時掃描對話，將超過保留期的非置頂對話自動封存。",
  "settings.archive.retention": "封存保留時長",
  "settings.archive.retentionDesc": "對話最後更新時間超過該時長後進入自動封存候選。",
  "settings.archive.afterDays": "{days} 天後封存",
  "settings.archived.desc": "查詢已封存的對話，並將其恢復到工作台。",
  "settings.archived.search": "搜尋已封存對話…",
  "settings.archived.empty": "暫無已封存對話",
  "settings.archived.noMatches": "沒有符合的已封存對話",
  "settings.archived.noProject": "對話",
  "settings.archived.restore": "恢復",
  "settings.archived.restoring": "正在恢復…",
  "settings.archived.delete": "徹底刪除",
  "settings.archived.deleteTitle": "刪除已封存對話",
  "settings.archived.deleteConfirm":
    "確定徹底刪除「{title}」及其全部訊息嗎？此操作無法復原。",
  "settings.archived.deleteDone": "已徹底刪除封存對話",
  "agent.streamStallCancel": "結束本輪",
  "agent.streamStallEndTurn": "結束本輪",
  "agent.streamStallKeepWaiting": "繼續等待",
  "settings.theme": "主題",
  "settings.themeDesc": "跟隨系統，或固定淺色 / 深色",
  "settings.themeSystem": "跟隨系統",
  "settings.themeLight": "淺色",
  "settings.themeDark": "深色",
  "settings.uiFontSize": "介面字號",
  "settings.uiFontSizeDesc":
    "統一縮放整個介面的文字，圖示與版面尺寸不受影響。",
  "settings.terminalFont": "終端字型",
  "settings.terminalFontDesc":
    "填寫 CSS 字型家族清單。Nerd Font 圖示需要此電腦已安裝對應字型。",
  "settings.skin": "配色皮膚",
  "settings.skinDesc": "強調色（可與淺色 / 深色疊加）",
  "settings.skin.default": "預設",
  "settings.skin.rose": "玫瑰",
  "settings.skin.gothic": "哥特",
  "settings.skin.mist": "薄霧",
  "settings.skin.ocean": "海洋",
  "settings.skin.ember": "餘燼",
  "settings.wallpaper": "背景圖",
  "settings.wallpaperDesc":
    "點擊預覽區選擇壁紙（JPEG/PNG/WebP、GIF 或 MP4/WebM）。靜態圖會壓縮後保存在本機；影片和 GIF 會作為動態背景播放。",
  "settings.wallpaperEmpty": "點擊選擇壁紙",
  "settings.wallpaperUpload": "選擇壁紙",
  "settings.wallpaperReplace": "更換壁紙",
  "settings.wallpaperClear": "移除",
  "settings.wallpaperWorking": "處理中…",
  "settings.wallpaperFocus": "調整位置",
  "settings.wallpaperFocusTitle": "背景位置",
  "settings.wallpaperFocusHint":
    "拖動與視窗同比例的取景框，選擇作為背景的區域；滾輪或滑桿可縮放。取景框比例會隨目前視窗變化。",
  "settings.wallpaperFocusHintVideo":
    "拖動取景框框選背景區域；用下方時間軸兩端把手選擇影片循環片段（不重編碼，僅按片段播放）。",
  "settings.wallpaperFocusZoom": "縮放",
  "settings.wallpaperClip": "片段",
  "settings.wallpaperClipStart": "片段起點",
  "settings.wallpaperClipEnd": "片段終點",
  "settings.wallpaperFocusReset": "重置",
  "settings.wallpaperFocusApply": "套用",
  "settings.wallpaperScrim": "遮罩強度",
  "settings.wallpaperVisibility": "背景可見度",
  "settings.wallpaperBlur": "背景模糊",
  "settings.wallpaperAppearanceReset": "恢復預設",
  "settings.wallpaperScrimDesc":
    "降低可見度或增加模糊可以讓文字更容易閱讀。",
  "settings.wallpaper.err.not_image": "請選擇圖片或影片檔。",
  "settings.wallpaper.err.too_large": "圖片過大（最大 12 MB）。",
  "settings.wallpaper.err.decode_failed": "無法讀取該圖片。",
  "settings.wallpaper.err.compress_failed": "無法壓縮該圖片。",
  "settings.wallpaper.err.still_too_large":
    "壓縮後仍過大，請換一張更小的圖。",
  "settings.wallpaper.err.unsupported_video": "僅支援 MP4 與 WebM 影片。",
  "settings.wallpaper.err.video_too_large": "影片過大（最大 200 MB）。",
  "settings.wallpaper.err.generic": "無法套用背景圖。",

  // 設定與自訂供應商。
  "settings.aboutApp": "關於 KeenCode",
  "settings.aboutTagline": "一款輕量、本機優先的桌面 AI 編碼工具。",
  "settings.aboutDescription":
    "為個人開發者打造，在一個專注的桌面工作台中完成程式碼理解、檔案修改、命令執行、變更審查與 Git 操作。",
  "settings.aboutLocalTitle": "本機優先",
  "settings.aboutLocalDesc":
    "專案、對話、設定與執行記錄預設保存在本機。程式碼不需要經過 KeenCode 強制提供的雲端服務。",
  "settings.aboutOpenTitle": "不綁定模型供應商",
  "settings.aboutOpenDesc":
    "KeenCode 不強制使用特定模型供應商，你可以依照自己的工作流程選擇和設定相容的 AI 服務。",
  "settings.aboutLightTitle": "為輕量而設計",
  "settings.aboutLightDesc":
    "桌面核心使用 Rust 編寫並重用系統 WebView，不綁定瀏覽器核心、Node.js 或模型權重。記憶體、CPU、啟動速度和安裝套件大小都是核心產品指標。",
  "settings.aboutSourceTitle": "開放原始碼",
  "settings.aboutSourceDesc":
    "採用 MIT 授權條款發布，建置流程與架構公開、可稽核、可重現。",
  "settings.updateTitle": "應用程式更新",
  "settings.updateSource": "更新來源",
  "settings.updateSourceDesc":
    "自動模式優先透過 ghfast.top 檢查並下載更新，失敗後嘗試 GitHub。加速服務可得知你的 IP、版本和平台，安裝套件仍須通過 KeenCode 簽章驗證。",
  "settings.updateSourceAuto": "自動",
  "settings.updateSourceGithub": "GitHub",
  "settings.updateSourceChinaMirror": "中國大陸加速",
  "settings.updateCurrentSource": "下載來源：{source}",
  "settings.updateIdle": "從 GitHub Releases 檢查新的 KeenCode 簽章版本。",
  "settings.updateChecking": "正在檢查 GitHub Releases…",
  "settings.updateInstalling": "正在安裝更新，完成後 KeenCode 會重新啟動。",
  "settings.updateAvailable": "發現新版本 {version}。",
  "settings.updateDownloading": "正在背景下載 {version}…",
  "settings.updateVerifying": "下載完成，正在驗證安裝套件簽章…",
  "settings.updateReady": "{version} 已下載並通過簽章驗證，可以安裝。",
  "settings.updateDownloadFailed": "未能準備好 {version}。",
  "settings.updateCurrent": "目前已是最新版本。",
  "settings.updateNotes": "版本說明",
  "settings.updateCheck": "檢查更新",
  "settings.updateInstall": "安裝並重新啟動",
  "settings.updateShowProgress": "查看進度",
  "settings.updateRetry": "重試下載",
  "settings.updateDownloadProgress": "更新下載進度",
  "settings.updateProgressKnown": "{downloaded} / {total} · {percent}%",
  "settings.updateProgressUnknown": "已下載 {downloaded}",
  "settings.updateBackgroundHint": "關閉此視窗不會中斷下載，KeenCode 會繼續在背景準備更新。",
  "settings.updateConfirmTitle": "安裝更新？",
  "settings.updateConfirm":
    "{version} 已下載並驗證完成。安裝會停止正在執行的任務並重新啟動 KeenCode，是否繼續？",
  "settings.updateCheckingAction": "檢查中…",
  "settings.updateInstallingAction": "安裝中…",
  "settings.tabProviders": "自訂供應商",
  "settings.tabProvidersHint":
    "新增支援 OpenAI Chat Completions、OpenAI Responses 或 Anthropic Messages 的模型供應商。金鑰僅保存在 KeenCode 本機應用程式設定中。",

  "prov.emptyTitle": "尚無供應商",
  "prov.detailEmpty": "選擇供應商，或新增一個。",
  "prov.new": "新增供應商",
  "prov.delete": "刪除",
  "prov.editTitle": "編輯供應商",
  "prov.addTitle": "新增供應商",
  "prov.name": "中文描述",
  "prov.namePh": "例如：工作中轉服務",
  "prov.baseUrl": "Base URL（/v1 可不填；結尾 # 表示完整端點）",
  "prov.baseUrlPh": "https://your-relay.example.com",
  "prov.protocol": "訊息格式",
  "prov.protocol.chatCompletions": "OpenAI Chat Completions",
  "prov.protocol.responses": "OpenAI Responses",
  "prov.protocol.messages": "Anthropic Messages",
  "prov.apiKey": "API Key",
  "prov.keyPh": "sk-…",
  "prov.keyStorageHint": "已儲存的金鑰會顯示在此；清空後儲存將移除該金鑰。",
  "prov.keyShow": "顯示",
  "prov.keyHide": "隱藏",
  "prov.modelList": "模型列表",
  "prov.modelCount": "{n} 個模型",
  "prov.modelListEmpty": "尚未新增模型",
  "prov.addModel": "新增模型",
  "prov.modelId": "模型名稱",
  "prov.modelAddHint": "新增至目前供應商的草稿清單，儲存供應商後生效。",
  "prov.modelEditHint": "修改先套用至草稿，儲存供應商後生效。",
  "prov.removeModel": "移除模型 {model}",
  "prov.maxOutputTokens": "最大輸出 Token",
  "prov.editModel": "編輯模型",
  "prov.applyModel": "套用修改",
  "prov.err.outputTokens": "最大輸出 Token 必須是 1 到 4294967295 之間的整數。",
  "prov.contextWindow": "上下文視窗（Token）",
  "prov.contextWindowFor": "{model} 的上下文視窗（Token）",
  "prov.contextWindowPh": "200000",
  "prov.supportsVision": "支援視覺",
  "prov.modelPickerTitle": "選擇模型",
  "prov.addSelected": "新增已選模型（{n}）",
  "prov.modelPh": "例如 gpt-5",
  "prov.fetchModels": "擷取模型",
  "prov.fetching": "擷取中…",
  "prov.selectAll": "全選",
  "prov.deselectAll": "取消全選",
  "prov.emptyList": "未回傳模型",
  "prov.save": "儲存",
  "prov.add": "新增",
  "prov.saving": "儲存中…",
  "prov.confirmDelete": "刪除供應商「{id}」？",
  "prov.loading": "載入中…",
  "prov.err.needBase": "請填寫 Base URL。",
  "prov.err.needDescription": "請填寫中文描述。",
  "prov.err.needModel": "請至少新增一個模型。",
  "prov.copy": "複製供應商資訊",
  "prov.copyDone": "供應商資訊已複製為 JSON。",
  "prov.copyFail": "複製失敗：{error}",
  "prov.exportOne": "匯出 JSON",
  "prov.exportDone": "供應商資訊已匯出。",
  "prov.exportFail": "匯出失敗：{error}",
  "prov.importAll": "匯入",
  "prov.importTitle": "匯入供應商",
  "prov.importHint":
    "選擇匯出的供應商 JSON 檔案。標識相同的供應商會被更新，其餘新增；不會改變目前使用的供應商。",
  "prov.importPickFile": "選擇檔案…",
  "prov.importPickHint": "僅支援 JSON 檔案",
  "prov.importParsed": "檔案中包含 {n} 個供應商。",
  "prov.importSubmit": "匯入",
  "prov.importWorking": "匯入中…",
  "prov.importDone": "已匯入 {added} 個供應商，更新 {updated} 個。",
  "prov.importFail": "匯入失敗：{error}",
  "prov.importErr.json": "檔案不是有效的 JSON。",
  "prov.importErr.schema": "檔案不是 KeenCode 供應商匯出格式（schema 不符）。",
  "prov.importErr.empty": "檔案中沒有可匯入的供應商。",
  "prov.switchedHotReload":
    "已切換模型供應商。",
  "common.cancel": "取消",
  "common.confirm": "確認",
  "common.save": "儲存",
  "common.dismiss": "關閉",



  "shortcuts.title": "鍵盤快捷鍵",
  "shortcuts.close": "關閉",
  "shortcuts.search": "搜尋對話 / 專案",
  "shortcuts.newChat": "新增對話",
  "shortcuts.settings": "設定",
  "shortcuts.stop": "停止產生 / 關閉浮層",
  "shortcuts.send": "傳送訊息",
  "shortcuts.help": "顯示快捷鍵",

  "session.exportDone": "對話已匯出",
  "session.exportFail": "匯出失敗",



  "slash.section.commands": "指令",
  "slash.empty": "沒有相符的指令或技能",
  "slash.goal": "Goal 模式",
  "slash.goalDesc": "將首則請求作為目前對話的持久目標持續推進",
  "slash.plan": "計劃模式",
  "slash.planDesc": "切換唯讀規劃：調研程式碼並產出實施計劃",
  "goal.clear": "清除目標",
  "goal.clearTitle": "清除目前目標",
  "goal.clearConfirm": "清除「{title}」？該目標將從目前對話中移除。",
  "goal.clearFailed": "清除目標失敗：{error}",
  "goal.editTitle": "編輯目前目標",
  "goal.editPlaceholder": "目標內容",
  "goal.editFailed": "編輯目標失敗：{error}",
  "goal.objectiveRequired": "目標模式需要填寫文字目標",
  "goal.objectiveTooLong": "目標正文不能超過 65536 個 UTF-8 位元組",
  "compact.bannerAuto": "上下文已自動壓縮",
  "compact.bannerManual": "上下文已壓縮",
  "compact.bannerRunning": "上下文壓縮中…",
  "compact.bannerFailed": "上下文壓縮失敗",
  "compact.tokensRange": "{before} → {after} tokens",
  "compact.summaryToggle": "摘要",
  "context.chipAria": "上下文用量",
  "context.usageRate": "上下文使用率",
  "context.taskCacheHitRate": "任務快取命中率",
  "activity.running": "進行中",
  "activity.done": "完成",
  "activity.failed": "失敗",
  "activity.cancelled": "已取消",
  "activity.cancelledByUser": "已由使用者停止",
  "activity.cancelledAgentExit": "Agent 程序已結束",
  "endOfTurn.stall": "已暫停 — 暫時沒有新輸出",
  "endOfTurn.stallPreToken": "已暫停 — 仍在等待首個模型輸出",
  "endOfTurn.stallWorkingTools": "已暫停 — 工具長時間無新進度",
  "endOfTurn.stallMaybeDone": "本輪可能已經完成",
  "endOfTurn.error": "本輪以錯誤結束",
  "endOfTurn.unknown": "本輪已結束",
  "timelinePhase.steps": "{n} 步",
  "timelinePhase.gistSteps": "{gist} · {n} 步",
  "timelinePhase.stepsWithErrors": "{n} 步 · {e} 失敗",
  "timelinePhase.gistStepsWithErrors": "{gist} · {n} 步 · {e} 失敗",
  "timelinePhase.running": "{n} 步（進行中…）",
  "timelinePhase.liveProgress": "正在工作 · {completed}/{n} 步",
  "timelinePhase.gistRunning": "{gist} · {n} 步…",
  "timelinePhase.working": "進行中…",
  "timelinePhase.failed": "{e} 失敗",
  "slash.status": "狀態",
  "slash.historyEmpty": "目前對話還沒有提示",
  "promptHistory.title": "目前對話",
  "promptHistory.placeholder": "篩選提示…",
  "promptHistory.empty": "目前對話還沒有提示",
  "promptHistory.emptyFilter": "沒有相符的提示",
  "promptHistory.aria": "目前對話的提示歷史",

  "statusModal.title": "對話狀態",
  "statusModal.sessionId": "對話 ID",
  "statusModal.model": "模型",
  "statusModal.effort": "推理",
  "statusModal.executionPolicy": "執行策略",
  "statusModal.executionPolicyValue": "自動執行，高風險指令硬性攔截",
  "statusModal.project": "專案",
  "statusModal.messages": "訊息數",


  // Settings → Extensions (Plugins + Skills + MCP)
  "ext.lead":
    "管理使用者外掛、使用者/專案技能與 KeenCode MCP 設定。",
  "ext.restartHint": "外掛與 MCP 變更會立即生效；外掛 LSP 伺服器變更需重新啟動 KeenCode。",
  "ext.refresh": "重新整理",
  "ext.refreshing": "重新整理中…",
  "ext.scope.project": "專案",
  "ext.scope.global": "使用者",
  "ext.scope.globalHint": "目前無活動專案 — 僅列出使用者範圍",
  "ext.reveal": "在檔案總管中顯示",
  "ext.needTauri": "擴充管理需要在 KeenCode 桌面應用程式中使用。",
  "ext.error.title": "無法檢查",
  "ext.plugins.title": "已安裝外掛",
  "ext.plugins.loading": "正在載入外掛…",
  "ext.plugins.empty": "尚未安裝外掛",
  "ext.plugins.browseMarket": "瀏覽外掛市集",
  "ext.plugins.status.enabled": "已啟用",
  "ext.plugins.status.disabled": "已停用",
  "ext.plugins.enable": "啟用",
  "ext.plugins.disable": "停用",
  "ext.plugins.details": "詳情",
  "ext.plugins.configure": "設定",
  "ext.plugins.configureTitle": "設定 {name}",
  "ext.plugins.configLoading": "正在載入外掛設定…",
  "ext.plugins.configEmpty": "此外掛沒有可設定欄位。",
  "ext.plugins.configSave": "儲存",
  "ext.plugins.configSaving": "儲存中…",
  "ext.plugins.configImmediate": "設定會儲存在本機並立即生效，不需要重新啟動應用程式。",
  "ext.plugins.configLspRestart":
    "設定會儲存在本機；LSP 伺服器設定會在重新啟動 KeenCode 後生效。",
  "ext.plugins.configSensitive": "敏感",
  "ext.plugins.configChoosePath": "選擇…",
  "ext.plugins.configAddValue": "新增值",
  "ext.plugins.configRemoveValue": "移除值",
  "ext.plugins.configUnset": "未設定",
  "ext.plugins.configBounds": "允許範圍：{min} – {max}",
  "ext.plugins.uninstall": "解除安裝",
  "ext.plugins.uninstallTitle": "解除安裝外掛",
  "ext.plugins.uninstallConfirm":
    "確定解除安裝「{name}」？其元件會立即停止載入。",
  "ext.plugins.uninstallConfirmLsp":
    "確定解除安裝「{name}」？其他元件會立即停止載入，已裝配的 LSP 伺服器會在重新啟動 KeenCode 後停止。",
  "ext.plugins.detailsTitle": "{name}",
  "ext.plugins.detailsLoading": "正在載入詳情…",
  "ext.plugins.detailsEmpty": "未回傳詳情。",
  "ext.plugins.marketplace": "市集",
  "ext.plugins.lspRestart":
    "LSP 伺服器會在 KeenCode 啟動時裝配；啟用、停用與設定變更需重新啟動後生效。",
  "ext.plugins.working": "處理中…",
  "ext.plugins.actionError": "外掛操作失敗",
  "ext.plugins.filterLabel": "篩選外掛",
  "ext.plugins.filter.all": "全部",
  "ext.plugins.filter.enabled": "已啟用",
  "ext.plugins.filter.disabled": "已停用",
  "ext.plugins.filterEmpty": "沒有符合篩選條件的外掛。",
  "ext.plugins.update": "更新",
  "ext.plugins.updateAll": "全部更新",
  "ext.plugins.updating": "正在更新…",
  "ext.skills.title": "技能",
  "ext.skills.loading": "正在載入技能…",
  "agents.title": "子智能體",
  "agents.lead": "管理所有專案共用的子智能體，並查看內建子智能體。",
  "agents.loading": "正在載入子智能體…",
  "agents.empty": "目前沒有可用子智能體",
  "agents.add": "新增子智能體",
  "agents.addTitle": "新增子智能體",
  "agents.name": "識別碼",
  "agents.description": "委派說明",
  "agents.prompt": "系統提示",
  "agents.tools": "可用工具",
  "agents.tools.all": "全部工具",
  "agents.tools.allHint": "繼承主智能體可用的全部工具",
  "agents.tools.specific": "指定工具",
  "agents.tools.specificHint": "選擇此子智能體可以呼叫的工具",
  "agents.tools.empty": "沒有可用工具",
  "agents.injectAgentsMd": "注入 AGENTS.md",
  "agents.injectAgentsMdHint": "將專案指令加入此子智能體的上下文",
  "agents.maxTurns": "最大輪數",
  "agents.create": "建立",
  "agents.creating": "建立中…",
  "agents.remove": "刪除",
  "agents.removeTitle": "刪除子智能體",
  "agents.removeConfirm": "確定刪除子智能體「{name}」？",
  "agents.model": "模型覆寫",
  "agents.model.assign": "指定模型",
  "agents.model.followSession": "跟隨目前工作階段",
  "agents.source.global": "使用者",
  "agents.source.builtin": "內建",
  "agents.source.plugin": "外掛",
  "agents.detail.view": "檢視定義",
  "agents.detailTitle": "子智能體詳情",
  "agents.detail.toolsInherit": "繼承主智能體的全部工具",
  "agents.detail.toolsNone": "無工具",
  "agents.detail.disallowed": "排除的工具",
  "agents.detail.sandboxDirs": "沙箱可寫目錄",
  "ext.skills.empty": "未發現技能",
  "ext.skills.invocable": "斜線指令",
  "ext.mcp.title": "MCP 伺服器",
  "ext.mcp.loading": "正在載入 MCP…",
  "ext.mcp.empty": "未發現 MCP 伺服器",
  "ext.mcp.runtime.pending": "執行階段待初始化",
  "ext.mcp.runtime.initializing": "執行階段初始化中",
  "ext.mcp.runtime.ready": "執行階段就緒",
  "ext.mcp.runtime.failed": "執行階段失敗",
  "ext.mcp.status.connected": "已連線",
  "ext.mcp.status.failed": "連線失敗",
  "ext.mcp.status.disconnected": "已中斷連線",
  "ext.mcp.status.disabled": "已停用",
  "ext.mcp.status.uninitialized": "尚未初始化",
  "ext.mcp.transport": "傳輸：{transport}",
  "ext.mcp.toolsCount": "{count} 個工具",
  "ext.mcp.oauth.authorized": "OAuth 已授權",
  "ext.mcp.oauth.needsAuthorization": "需要 OAuth 授權",
  "ext.mcp.authorize": "授權",
  "ext.mcp.authorizing": "正在發起授權…",
  "ext.mcp.cancelAuth": "取消授權",
  "ext.mcp.canceling": "正在取消…",
  "ext.mcp.oauthOpenFailed": "無法開啟系統瀏覽器",
  "ext.mcp.oauthCallback.label": "手動完成 OAuth 回呼",
  "ext.mcp.oauthCallback.placeholder": "貼上回呼 URL、查詢參數或授權碼",
  "ext.mcp.oauthCallback.hint": "自動 localhost 回呼未完成時使用此入口。",
  "ext.mcp.oauthCallback.reopen": "重新開啟授權頁面",
  "ext.mcp.oauthCallback.submit": "提交回呼",
  "ext.mcp.oauthCallback.submitting": "正在提交…",
  "ext.mcp.oauthCallback.error.empty": "請貼上回呼 URL 或授權碼",
  "ext.mcp.oauthCallback.error.missingCode": "回呼中缺少授權碼",
  "ext.mcp.oauthCallback.error.missingState": "回呼中缺少 OAuth state，請貼上完整回呼 URL",
  "ext.mcp.oauthCallback.error.stateMismatch": "回呼 state 與本次授權請求不相符",
  "ext.mcp.add": "新增伺服器",
  "ext.mcp.addTitle": "新增 MCP 伺服器",
  "ext.mcp.addSubmit": "新增",
  "ext.mcp.addWorking": "正在新增…",
  "ext.mcp.addMode": "MCP 伺服器輸入方式",
  "ext.mcp.addModeForm": "手動填寫",
  "ext.mcp.addModeJson": "貼上 JSON",
  "ext.mcp.jsonLabel": "JSON 設定",
  "ext.mcp.jsonPlaceholder": "{\"mcpServers\":{\"playwright\":{\"command\":\"npx\",\"args\":[\"-y\",\"@playwright/mcp@latest\"]}}}",
  "ext.mcp.jsonHint": "支援 {\"server-name\":{…}} 和 {\"mcpServers\":{\"server-name\":{…}}}，可一次貼上多個伺服器。",
  "ext.mcp.jsonErrorEmpty": "請貼上 MCP JSON 設定。",
  "ext.mcp.jsonErrorInvalid": "MCP 設定不是有效的 JSON。",
  "ext.mcp.jsonErrorShape": "MCP JSON 必須是物件，或包含 mcpServers 物件。",
  "ext.mcp.jsonErrorEmptyServers": "MCP JSON 中沒有伺服器設定。",
  "ext.mcp.jsonErrorServer": "每個 MCP 伺服器都必須有非空名稱和物件設定。",
  "ext.mcp.name": "名稱",
  "ext.mcp.namePlaceholder": "例如 playwright",
  "ext.mcp.command": "命令",
  "ext.mcp.commandPlaceholder": "例如 npx",
  "ext.mcp.args": "參數",
  "ext.mcp.argsPlaceholder": "例如 -y @playwright/mcp@latest",
  "ext.mcp.argsHint": "以空格分隔的命令參數。",
  "ext.mcp.env": "環境變數（選用）",
  "ext.mcp.envPlaceholder": "KEY=value（每行一個）",
  "ext.mcp.envHint": "選用的行程環境變數。請勿貼上無法輪替的密鑰。",
  "ext.mcp.remove": "移除",
  "ext.mcp.removeTitle": "移除 MCP 伺服器",
  "ext.mcp.removeConfirm":
    "從 KeenCode MCP 設定中移除「{name}」？",
  "ext.mcp.doctor": "診斷",
  "ext.mcp.doctorTitle": "MCP 診斷",
  "ext.mcp.doctorRunning": "正在執行 MCP 診斷…",
  "ext.mcp.doctorEmpty": "未回傳任何 MCP 伺服器。",
  "ext.mcp.doctorRerun": "重新執行",
  "ext.mcp.doctorSummary":
    "{healthy} 健康 · {unhealthy} 異常 · 共 {total}",
  "ext.mcp.doctorHealthy": "健康",
  "ext.mcp.doctorUnhealthy": "異常",
  "ext.mcp.doctorSources": "設定來源",
  "ext.mcp.actionError": "MCP 操作失敗",
  "ext.enabled": "已啟用",
  "ext.disabled": "已停用",
  "ext.enableAll": "全部啟用",
  "ext.market.update": "重新整理",
  "ext.market.title": "外掛",
  "ext.market.loading": "正在載入市集…",
  "ext.market.empty": "尚未設定市集來源",
  "ext.market.addLabel": "新增來源",
  "ext.market.add": "新增",
  "ext.market.adding": "新增中…",
  "ext.market.addEmpty": "請輸入本機目錄、GitHub 倉庫、套件來源或市集 JSON URL。",
  "ext.market.remove": "移除",
  "ext.market.removeTitle": "移除市集來源",
  "ext.market.removeConfirm":
    "移除來源 “{name}”？已安裝的外掛需另行解除安裝。",
  "ext.market.updateAll": "重新整理來源",
  "ext.market.updating": "重新整理中…",
  "ext.market.refreshCatalog": "重新整理目錄",
  "ext.market.filterLabel": "依來源篩選",
  "ext.market.filterAll": "全部來源",
  "ext.market.sourcesTitle": "市集來源",
  "ext.market.showMore": "顯示更多（{n}）",
  "ext.market.availableEmpty": "沒有符合篩選的外掛。",
  "ext.market.availableLoading": "正在載入可用外掛…",
  "ext.market.install": "安裝",
  "ext.market.installing": "安裝中…",
  "ext.market.installTitle": "安裝外掛",
  "ext.market.installConfirm":
    "安裝並啟用 “{name}”？其技能會立即註冊，下一個任務即可使用。",
  "ext.market.installConfirmLsp":
    "安裝並啟用 “{name}”？其中包含 {count} 個 LSP 伺服器，安裝後需重新啟動 KeenCode 才會裝配。",
  "ext.market.lspRestart":
    "包含 {count} 個 LSP 伺服器；安裝後需重新啟動 KeenCode 才會裝配。",
  "ext.market.searchPlaceholder": "搜尋外掛…",
  "ext.market.paginationLabel": "外掛市集分頁",
  "ext.market.previousPage": "上一頁",
  "ext.market.nextPage": "下一頁",
  "ext.market.pageStatus": "第 {page} / {pages} 頁",
  "ext.market.error": "市集操作失敗",
  "ext.compatibility.title": "適配設定",
  "ext.compatibility.description": "設定外掛 Agent 的模型別名。預設繼承目前模型；所選供應商或模型被刪除後，也會自動繼承目前模型。",
  "ext.compatibility.loadFailed": "無法載入適配設定，請關閉後重新開啟。",
  "ext.compatibility.saveFailed": "無法儲存適配設定，請重試。",
  "ext.market.installed": "已安裝",
  "ext.market.manage": "管理",
  "ext.market.loadFailed": "外掛市集載入失敗。請檢查網路連線或系統代理，然後點擊重新整理以重試。",
  "ext.market.addFailed": "新增市集失敗。請檢查來源位址和網路連線後重試。",
  "ext.market.removeFailed": "移除市集失敗。請重新整理後重試。",
  "ext.market.installFailed": "外掛安裝失敗。請檢查外掛來源和網路連線後重試。",
"error.needTauri": "需要在 Tauri 視窗中選擇目錄",
  "common.close": "關閉",
  "a11y.assistantStreaming": "助手正在回覆",
  "a11y.assistantDone": "助手回覆已完成",

  "askUser.title": "Agent 提問",
  "askUser.submit": "提交",
  "askUser.cancel": "略過",
  "askUser.otherPlaceholder": "輸入你的回答…",
  "askUser.freeTextHint": "或輸入自訂回答",
  "askUser.multiHint": "可多選",
  "askUser.next": "下一步",

  // 會話摘要面板
  "summary.title": "摘要",
  "summary.back": "返回",
  "summary.refresh": "重新整理摘要",
  "summary.changes": "變更",
  "summary.filesChanged": "{count} 個檔案已變更",
  "summary.branch": "目前分支",
  "summary.branchUnavailable": "不可用",
  "summary.branches.open": "選擇分支",
  "summary.branches.search": "搜尋分支",
  "summary.branches.title": "分支",
  "summary.branches.uncommitted": "未提交：{count} 個檔案",
  "summary.branches.create": "建立並簽出新分支…",
  "summary.branches.createTitle": "建立並簽出分支",
  "summary.branches.name": "分支名稱",
  "summary.branches.invalid": "分支名稱不能為空或以「/」結尾。",
  "summary.branches.createAction": "建立並簽出",
  "summary.commitOrPush": "提交或推送",
  "summary.git.messageRequired": "請先填寫提交訊息。",
  "summary.git.committed": "已提交 {commit}",
  "summary.git.committedAndPushed": "已提交 {commit} 並推送",
  "summary.git.pushed": "已推送到遠端",
  "summary.git.commitThenPushFailed": "已提交 {commit}，但推送失敗：{error}",
  "summary.git.message": "提交訊息",
  "summary.git.messagePlaceholder": "填寫提交訊息",
  "summary.git.includeUnstaged": "包含未暫存變更",
  "summary.git.commit": "提交",
  "summary.git.commitAndPush": "提交並推送",
  "summary.git.push": "推送",
  "summary.backgroundShells.title": "背景程序",
  "summary.backgroundShells.stop": "停止背景程序",
  "summary.subagents.title": "子智能體",
  "summary.subagents.processing": "處理中…",
  "summary.subagents.noActivity": "暫無活動",
  "summary.subagents.noneRunning": "暫無執行中的子智能體",
  "summary.subagents.stop": "停止子智能體「{name}」",
  "summary.subagents.stopFailed": "停止子智能體失敗：{error}",
  "summary.subagents.running": "執行中（{count}）",
  "summary.subagents.failed": "需要處理（{count}）",
  "summary.subagents.interrupted": "已中斷（{count}）",
  "summary.subagents.completed": "已完成（{count}）",
  "summary.empty": "暫無摘要內容",

  // 設定 → 個人化
  "settings.personalization.customInstructions": "全域自訂指令",
  "settings.personalization.description": "儲存後從下一輪對話生效，無需重新啟動。",
  "settings.personalization.learnMore": "了解更多",
  "settings.personalization.help":
    "KeenCode 會在每輪主 Agent 或子 Agent 執行前讀取此檔案。使用遠端模型時，內容會隨請求傳送給你設定的模型供應商。",
  "settings.personalization.placeholder": "例如：一律用繁體中文回覆，並先說明程式碼思路…",
  "settings.personalization.saveFailed": "儲存失敗，請重試。",
  "settings.personalization.memories": "記憶",
  "settings.personalization.memoriesDescription":
    "設定在此電腦上如何收集、保留和整合本機記憶。",
  "settings.personalization.enableMemories": "啟用本機記憶",
  "settings.personalization.enableMemoriesDescription":
    "根據此電腦上的聊天建立記憶，並用於個人化此電腦上的未來聊天。",
  "settings.personalization.longTermMemory": "長期記憶",
  "settings.personalization.deleteMemories": "刪除本機記憶",
  "settings.personalization.deleteMemoriesConfirm":
    "確定刪除此電腦上的全部本機記憶嗎？現有聊天不會被刪除。",
  "settings.personalization.deleteMemoriesDone": "已刪除本機記憶",
  "settings.personalization.memoriesFailed": "本機記憶操作失敗，請重試。",

  "settings.requests.description":
    "檢視本機請求的中繼資料、狀態、重試、耗時與 Token 數，不顯示請求或回應正文。",
  "settings.requests.loading": "正在載入請求記錄…",
  "settings.requests.error": "無法載入請求記錄",
  "settings.requests.empty": "目前篩選條件下沒有請求記錄。",
  "settings.requests.refresh": "重新整理",
  "settings.requests.refreshing": "正在重新整理…",
  "settings.requests.invalidDateRange": "開始日期不能晚於結束日期。",
  "settings.requests.filters": "篩選",
  "settings.requests.model": "模型",
  "settings.requests.status": "狀態",
  "settings.requests.from": "開始日期",
  "settings.requests.to": "結束日期",
  "settings.requests.allModels": "全部模型",
  "settings.requests.allStatuses": "全部狀態",
  "settings.requests.clearFilters": "清除篩選",
  "settings.requests.time": "時間",
  "settings.requests.provider": "服務商",
  "settings.requests.requestMode": "模式",
  "settings.requests.stream": "串流",
  "settings.requests.sync": "同步",
  "settings.requests.attempt": "嘗試",
  "settings.requests.duration": "耗時",
  "settings.requests.tokens": "Token",
  "settings.requests.details": "詳情",
  "settings.requests.purpose": "用途",
  "settings.requests.protocol": "協定",
  "settings.requests.endpoint": "Endpoint",
  "settings.requests.logicalRequestId": "邏輯請求 ID",
  "settings.requests.sessionId": "工作階段 ID",
  "settings.requests.turnId": "回合 ID",
  "settings.requests.agentId": "Agent ID",
  "settings.requests.firstResponse": "首次回應",
  "settings.requests.firstResponseDuration": "首次回應耗時",
  "settings.requests.completedAt": "完成時間",
  "settings.requests.httpStatus": "HTTP 狀態",
  "settings.requests.providerRequestId": "服務商請求 ID",
  "settings.requests.errorKind": "錯誤類型",
  "settings.requests.errorDetail": "錯誤",
  "settings.requests.cacheCreation": "快取寫入 Token",
  "settings.requests.cacheRead": "快取讀取 Token",
  "settings.requests.inputTokens": "輸入 Token",
  "settings.requests.outputTokens": "輸出 Token",
  "settings.requests.reasoningTokens": "推理 Token",
  "settings.requests.notReported": "未回報",
  "settings.requests.previous": "上一頁",
  "settings.requests.next": "下一頁",
  "settings.requests.range": "{from}–{to} / {total}",
  "settings.requests.statusSuccess": "成功",
  "settings.requests.statusRunning": "進行中",
  "settings.requests.statusFailed": "失敗",
  "settings.requests.statusCancelled": "已取消",
  "settings.requests.statusConnection": "連線失敗",
  "settings.requests.statusTimeout": "逾時",
  "settings.requests.statusTls": "TLS 錯誤",
  "settings.requests.statusTransport": "傳輸錯誤",
  "settings.requests.statusHttp": "HTTP 錯誤",
  "settings.requests.statusProtocol": "協定錯誤",
  "settings.requests.statusStreamInterrupted": "串流回應中斷",
  "settings.requests.statusRetryExhausted": "重試耗盡",
  "settings.requests.statusOther": "其他錯誤",

  "settings.analytics.loading": "正在載入用量資料…",
  "settings.analytics.empty": "尚無用量資料，開始對話後自動收集。",
  "settings.analytics.totalRequests": "總請求數",
  "settings.analytics.totalTokens": "總 Token 數",
  "settings.analytics.byModel": "依模型",
  "settings.analytics.byDay": "依天",
  "settings.analytics.activityHeatmap": "活躍熱力圖",
  "settings.analytics.less": "更少",
  "settings.analytics.more": "更多",
  "settings.analytics.tokenTrend": "Token 趨勢",
  "settings.analytics.modelUsage": "模型用量",
  "settings.analytics.rounds": "輪次",

  "settings.observability.title": "執行階段觀測",
  "settings.observability.description":
    "查看僅保存在本機且有界的執行階段指標、追蹤、啟動階段、資源與崩潰摘要。",
  "settings.observability.refresh": "重新整理",
  "settings.observability.refreshing": "正在重新整理…",
  "settings.observability.export": "匯出脫敏 JSON",
  "settings.observability.exporting": "正在匯出…",
  "settings.observability.loading": "正在載入執行階段觀測…",
  "settings.observability.unavailable": "執行階段觀測目前無法使用。",
  "settings.observability.events": "事件",
  "settings.observability.traces": "追蹤",
  "settings.observability.ttftP50": "首字節延遲 p50",
  "settings.observability.ttftP95": "首字節延遲 p95",
  "settings.observability.resources": "資源取樣",
  "settings.observability.crashes": "崩潰記錄",
  "settings.observability.dropped": "即時事件丟棄數",
  "settings.observability.startup": "啟動階段",
  "settings.observability.latestResource": "最近資源取樣",
  "settings.observability.latestTrace": "最近追蹤",
  "settings.observability.latestCrash": "最近崩潰",
  "settings.observability.noData": "未回報",
  "settings.observability.noCrashes": "尚無崩潰記錄",
  "settings.observability.noResources": "尚無資源取樣",
  "settings.observability.noStartup": "尚無啟動階段",
  "settings.observability.metric": "追蹤名稱",
  "settings.observability.count": "數量",
  "settings.observability.average": "平均值",
  "settings.observability.range": "範圍",
  "settings.observability.cpu": "CPU",
  "settings.observability.processMemory": "程序記憶體",
  "settings.observability.privateMemory": "私有記憶體",
  "settings.observability.processCount": "追蹤程序數",
  "settings.observability.frontendMemory": "前端堆積記憶體",
  "settings.observability.domNodes": "DOM 節點",
  "settings.observability.eventLoopLag": "事件迴圈延遲",
  "settings.observability.longTasks": "長工作",
  "settings.observability.phase": "階段",
  "settings.observability.elapsed": "耗時",
  "settings.observability.status": "狀態",
  "settings.observability.time": "時間",

  "message.copy": "複製",
  "message.edit": "編輯並重新傳送",
  "message.editInput": "編輯訊息",
  "message.expand": "展開訊息",
  "message.collapse": "收起訊息",
  "message.send": "傳送",
  "message.sending": "傳送中…",
  "message.copied": "已複製",
  "chat.thinking": "思考中…",
  "chat.thought": "思考",
  "chat.thinkingProcess": "思考過程",
  "chat.lastedFor": "持續了 {duration}",
  "chat.workingFor": "工作中 {duration}",
  "chat.workedFor": "已工作 {duration}",
  "chat.modelSwitched": "⇄ 模型已切換 {previous} → {model}",
  "chat.retryingAttempt": "正在進行第 {attempt}/{max} 次請求嘗試",
  "chat.turnMetrics.label": "本輪延遲",
  "chat.turnMetrics.usage": "本輪用量",
  "chat.turnMetrics.usageValue": "用量 {value}",
  "chat.turnMetrics.time": "本輪用時",
  "chat.turnMetrics.timeValue": "用時 {value}",
  "chat.turnMetrics.input": "輸入 Token（含快取）",
  "chat.turnMetrics.output": "輸出 Token",
  "chat.turnMetrics.reasoning": "推理 Token（包含在輸出中）",
  "chat.turnMetrics.cacheRead": "快取讀取 Token",
  "chat.turnMetrics.outputSpeed": "輸出速度（TPS）",
  "chat.turnMetrics.unknown": "未回報",
  "chat.turnMetrics.noUsage": "部分請求未回報用量，無法確定完整的本輪總量。",
  "chat.turnMetrics.notObserved": "本輪未記錄用量統計。",
  "tool.imageViewed": "已檢視 1 張圖像",
  "tool.imagesViewed": "已檢視 {count} 張圖像",
  "tool.imageNumber": "圖像 {count}",
  "chat.turnMetrics.totalTime": "總用時",
  "chat.turnMetrics.firstTokenLabel": "首 Token 延遲",
  "chat.turnMetrics.seconds": "{seconds}秒",
  "chat.turnMetrics.minutes": "{minutes}分{seconds}秒",
  "chat.turnMetrics.acknowledged": "傳送確認 {value}",
  "chat.turnMetrics.firstSse": "首 SSE {value}",
  "chat.turnMetrics.firstToken": "首 Token {value}",
  "chat.turnMetrics.completed": "完成 {value}",
  "chat.turnFailed": "本輪執行失敗",
  "chat.error.streamInterrupted":
    "模型回應在開始輸出後意外中斷。請重試；如果問題持續發生，請檢查網路，或切換供應商或模型。",
  "chat.error.modelRequestFailed":
    "模型請求失敗。請重試；如果問題持續發生，請檢查供應商或模型狀態。",
  "chat.error.modelHttp": "模型服務拒絕了請求。請檢查供應商或模型設定後重試。",
  "chat.error.internal": "KeenCode 遇到內部錯誤。請重試。",
  "chat.error.runtime": "Agent 執行階段意外停止。請重新連線後重試。",
  "chat.error.serialization": "KeenCode 無法處理回應。請重試。",
  "chat.error.maxIterations": "任務在完成前達到了最大執行步數。請傳送後續訊息繼續。",
  "chat.error.toolNotFound": "請求的工具無法使用。請重試任務或改用其他方式。",
  "chat.error.toolExecution": "工具未能完成操作。請查看工具詳細資訊後重試。",
  "chat.error.middleware": "任務擴充功能執行失敗。請停用相關擴充功能後重試。",
  "chat.error.toolRejected": "工具操作遭到拒絕。請查看工具詳細資訊後重試。",
  "chat.error.compactUnavailable":
    "KeenCode 無法壓縮對話內容。請新增對話或選擇其他模型。",
  "chat.error.compactEmpty": "模型傳回了空的內容摘要。請重試或選擇其他模型。",
  "chat.error.pendingTools": "有 {count} 個工具呼叫在完成前中斷，結果狀態未知。",
  "chat.error.generic": "操作失敗，請重試。",
  "chat.error.providerNotConfigured":
    "此工作階段的模型無法使用。請在設定中檢查供應商和模型，再重新選擇模型。",
  "chat.error.providerReloadFailed":
    "無法載入模型設定。請在設定中檢查供應商設定後重試。",
  "chat.system.statusChanged": "已連線服務的狀態發生變化。",
  "chat.system.mcpConnected": "MCP 伺服器 {name} 已連線，可用工具 {count} 個。",
  "chat.system.mcpFailed": "MCP 伺服器 {name} 執行失敗。請檢查其設定後重試。",
  "chat.system.mcpDisconnected": "MCP 伺服器 {name} 已中斷連線。",
  "chat.system.mcpDisabled": "MCP 伺服器 {name} 已停用。",
  "chat.system.mcpUninitialized": "MCP 伺服器 {name} 尚未初始化。",
  "chat.scrollBottom": "回到最新",
  "chat.codeLangText": "純文字",
  "chat.codeWrap": "啟用自動換行",
  "chat.codeUnwrap": "取消自動換行",
  "chat.codePreview": "開啟圖表預覽",
  "chat.codePreviewTitle": "圖表預覽",
  "chat.codePreviewDescription": "互動式圖表預覽",
  "chat.codePreviewLoading": "正在渲染圖表…",
  "chat.codePreviewZoomIn": "放大",
  "chat.codePreviewZoomOut": "縮小",
  "chat.codePreviewReset": "重設縮放",
  "sidebar.newSession": "新增對話",

  "composer.worktreeGc": "清理失效 worktree",
  "composer.worktreeGcTitle": "清理失效 worktree",
  "composer.worktreeGcHint": "清理已刪除目錄對應的 worktree 管理記錄（git worktree prune）。不會刪除仍在使用的 worktree 或分支。",
  "composer.worktreeGcForce": "立即清理全部年齡（--expire now）",
  "composer.worktreeGcPreview": "預覽",
  "composer.worktreeGcPreviewLoading": "正在掃描…",
  "composer.worktreeGcPrunable": "列表中有 {n} 個可修剪 worktree",
  "composer.worktreeGcPreviewEmpty": "未發現可清理的失效記錄。",
  "composer.worktreeGcRunning": "正在清理…",
  "composer.worktreeGcConfirm": "清理",
  "composer.worktreeGcDone": "已清理 {n} 條失效 worktree 記錄",
  "composer.worktreeGcDoneNone": "沒有需要清理的內容",
};
