...
function createHarness() {
  ...
  const options: UseSessionNavigationOptions = {
    ...
    sidebar: { projects: [], activeProject: null, setActiveProject: vi.fn(), setExpandedProjects: vi.fn(), setHistoryOpen: vi.fn(), setUnreadTerminalResults: vi.fn(), pendingAskUserBySessionRef: { current: new Map() } },
  ...
  return { navigation, options, setSessionModelReference };
}

afterEach(() => vi.unstubAllGlobals());

it("从备用模型会话新建草稿时恢复保存的主模型，保留原会话缓存", async () => {
  const { navigation, options, setSessionModelReference } = createHarness();
  await navigation.newChat(null);
  // 默认模型未声明供应商，引用退化为纯模型 ID。
  expect(setSessionModelReference).toHaveBeenCalledWith("hy3");
