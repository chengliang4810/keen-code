...
- `sections/05_using_tools.md`（工具错误段）：新增失败分类协议——瞬态失败（超时/限流/暂时不可用）须重试同一调用直至成功，不因一两次失败放弃可行做法；永久失败（参数无效/权限拒绝/资源不存在）重试无意义，停止该动作并如实报告与给替代方案。归因：重试毅力与错误分类属于通用工具使用纪律。
- `sections/03_doing_tasks.md`（任务执行段）：新增压缩前落盘——工具结果与旧消息在压缩时可能被替换为占位文本，趁还在时把后续需要的标识、代码、路径与关键数字写进回复或工作区文件。归因：Micro 压缩会替换旧工具结果，模型侧需要对应的留存义务。

曾同批加入的「守护文件禁令」（不得改检查与其输入让检查变绿）在验证中未能改变目标行为（六个任务同分复现），且其措辞面向评测判分语义、偏离通用产品提示词定位，本次撤销；该行为的治理转入机制层：任务声明的受保护文件经由沙箱 write-deny 由工具硬性拒绝，见 `core/tools` 沙箱写保护。
