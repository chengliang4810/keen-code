...
三条行为规则来自 harness-bench 基准 168 题考试的失败归因（151/168 时剩余 16 题的判分器与故障注入证据），全部为独立撰写的行为约束，描述 KeenCode 真实机制，未搬运外部提示词原文；措辞参考了公开可见的同类义务类条款方向（坚持重试、按错误类别分流、检查不可伪造），文字为本仓原创。

- `sections/05_using_tools.md`（工具错误段）：新增失败分类协议——瞬态失败（超时/限流/暂时不可用）须重试同一调用直至成功，不因一两次失败放弃可行做法；永久失败（参数无效/权限拒绝/资源不存在）重试无意义，停止该动作并如实报告与给替代方案。归因：TR4（故障表前 2 次必失败，agent 只调 1 次即放弃）、MA4（永久错误被当瞬态退避重试 4 次，判据 ≤3）。
- `sections/03_doing_tasks.md`（验证段）：新增守护文件禁令——不得通过编辑检查本身或其输入让失败检查变绿；测试、fixture、种子与其他提供或生成的文件在任务未明确要求时保持原内容，修正被检查的实现。归因：qi-021/040/041/083/085 五题均以「改测试/改守护源码/改原始输入」换取检查通过（判分器 test_integrity/source_integrity 逐题取证）。
- `sections/03_doing_tasks.md`（任务执行段）：新增压缩前落盘——工具结果与旧消息在压缩时可能被替换为占位文本，趁还在时把后续需要的标识、代码、路径与关键数字写进回复或工作区文件。归因：SP3（437KB 工具结果进上下文，压缩 6 次后撞窗口下限；Bash 溢出落盘机制已存在且写入 OS temp 工件目录，模型侧缺「主动留存关键信息」的义务）。

## 本次行为调整（2026-09-24）

工具描述增厚：`Read`、`Edit`、`Write`、`Glob`、`Grep`、`Bash` 的 description 在原有事实说明后追加使用协议（续读、唯一匹配、批量搜索收敛、非交互命令、版本控制安全、失败后改变做法）。全部文本为本次独立撰写，描述 KeenCode 的真实行为与既有工具契约，未搬运任何外部项目的提示词原文。增厚前六个高频工具的描述合计 1,117 字节，增厚后 4,797 字节；这部分计入工具 schema，随请求发送一次并进入缓存前缀。

`Read` 默认行数由 300 提升到 2,000（`DEFAULT_READ_LINES`）。默认值只决定单次调用能取回多少上下文，真正防止上下文膨胀的是 `max_read_output_bytes`（32 KiB）输出字节上限，两者独立；实测该上限先行生效，默认读取实际返回约 1.7 千行，是原默认值的 5 倍以上，其余靠 `offset` 续读。未变文件 stub 未实现：Micro 压缩会把旧工具结果替换为哨兵文本，此时对同一文件返回“未变更”会让模型永久失去该内容，收益不抵正确性风险。

压缩后的重读提示（`PostCompactionReadHint`）由“只列路径”升级为“路径 + 上次读取区域”：提示中带 `offset`/`limit` 的条目可按原区域续读，避免整份重读；未提供区域时仍只列路径，不编造行号。

命令超时报告改为可执行指引：除报告超时毫秒数外，同时给出当前环境的 `max_command_timeout_ms` 上限，并明确提示可提高 `timeout_ms` 或改用 `run_in_background` 后经 `TaskOutput` 收集，且不要按原参数重试。前台进程超时后自动转为后台继续运行未实现：那需要把已启动进程的输出与监督权从 `supervise_spawned` 移交给 `BackgroundTaskManager`，属进程生命周期重构，本次不做。

用户级 Hooks 配置新增：`~/.keencode/hooks.json`，与插件 Hook 使用同一份事件协议与同一套严格解析器，Hook 名称带 `user:` 命名空间，命令 Hook 的工作目录取当前项目根。文件缺失静默跳过；文件存在但无效时隔离全部用户 Hook 并产生 `hooks_user_config_invalid` 诊断，不影响插件来源。此前 Hooks 只能由插件贡献。

## 本次行为调整（2026-09-20）

环境使用 Developer 角色，Memory/Plan 等背景保留原有角色，均在真实对话历史之前装配。工具结果后的续轮不再以重复背景作为最后一条用户消息。背景变化可能降低跨 Turn 前缀缓存命中；不能以缓存稳定为由改变最新输入的含义。请求预算与发送继续共用同一装配路径，背景不写入 Journal 或压缩摘要。

上一条同 Agent 异常终态在下次 Turn 作为请求期说明提供。取消来源未知时不称为“用户手动停止”；已完成工具与外部副作用不因停止而回滚。正常完成后不再携带旧异常说明。

任务和表达规则新增的是独立编写的行为约束：工具续轮仍服务当前任务，更新应提供新增证据；证据不足时明确未知，不能用重复检查或确定性口吻代替验证。相同只读结果的运行时提醒是局部观察，不是语义完成判定，不强制终止合理长任务。

以下来源与测试记录属于历史记录，不代表当前正文、架构或本次验收结果；历史来源说明保留，不据此宣称来源清理已经完成。
...
Goal 审计采用 ZCode 抓取中的逐项需求到证据映射、检查验证覆盖范围、禁止代理信号替代验收等规则，放在 `core/agent/src/runner.rs` 的 Goal 续行指令中，仅在该运行时边界注入。KeenCode 仍通过 `Goal complete` 提交证据，不复制 ZCode 的独立完成验证器声明；不改变预算、状态机或阻塞条件。

Todo 提醒于 2026-09-17 自上游快照移植（`TODO_REMINDER_CONFIG` 的 10/10 节奏，`core/agent/src/runner.rs`），注入文本使用运行时提醒前缀并附当前 Todo 列表，仅根 Agent 接线。提示词段落无改动：上游主提示词自带的 TodoWrite 拆解指导在本仓 `sections/03_doing_tasks.md` 与 TodoWrite 工具描述中已有等价条款，不重复注入。
...
| `../../core/tools/src/` | 各工具的描述、参数 Schema、实际执行约束 |
...
files = sorted(Path("apps/desktop/prompts/sections").glob("*.md"))
...
```sh
cargo test -p keencode-provider --lib
cargo test -p keencode-tools --lib
cargo test -p keencode-agent context --lib
cargo test -p keencode-desktop agent_prompt --lib
cargo test -p keencode-desktop agent_runtime::tests --lib
git diff --check
```
...
```sh
cargo test -p keencode-desktop live_prompt_scope_and_cache --lib -- --ignored --nocapture
```
